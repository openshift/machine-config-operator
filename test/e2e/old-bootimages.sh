...
oldboot_ms=$(oc -n openshift-machine-api -o name get machinesets | grep oldboot || true)
...
for rel in ${oldreleases[@]}; do
  ...
  runverbose oc -n openshift-machine-api create -f ${targetname}.json
  oc -n openshift-machine-api get -o yaml machineset/"${targetname}" > targetname-out.yaml
...
for x in $(seq 60); do
    for rel in ${oldreleases[@]}; do
        ...
        machine=$(oc -n openshift-machine-api get -o name machine | grep oldboot-${rel} || true)
