...
func TestOSDetection(t *testing.T) {
	...
	for _, node := range nodes.Items {
		...
		t.Run("", func(t *testing.T) {
			...
			switch {
			case strings.Contains(nodeOSRelease.EtcContent, rhcos) && strings.Contains(nodeOSRelease.EtcContent, rhelVersion8):
			...
			case strings.Contains(nodeOSRelease.EtcContent, rhcos) && strings.Contains(nodeOSRelease.EtcContent, rhelVersion9):
				rhelVersion := nodeOSRelease.OS.Values()["RHEL_VERSION"]
			...
			case strings.Contains(nodeOSRelease.EtcContent, fcos):
				...
				t.Logf("Identified %s on node %s", fcos, node.Name)
			default:
