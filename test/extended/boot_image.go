...
func verifyMachineSetUpdate(oc *exutil.CLI, machineSet machinev1beta1.MachineSet, updateExpected bool) {
	...
	defer func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		framework.Logf("Restored build name in the machineset %s from \"%s\" to \"%s\"", machineSet.Name, backdatedBootImage, originalBootImage)
	}()
...
func getAzureDiskReleaseFromConfigMap(configMap *corev1.ConfigMap, arch string) (string, error) {
	streamData, exists := configMap.Data["stream"]
	if !exists {
		return "", fmt.Errorf("stream data not found in ConfigMap")
	}

	var streamJSON map[string]any
	if err := json.Unmarshal([]byte(streamData), &streamJSON); err != nil {
		return "", fmt.Errorf("failed to unmarshal stream data: %v", err)
	}

	// Navigate to architectures[arch].rhel-coreos-extensions.azure-disk.release
	architectures, exists := streamJSON["architectures"].(map[string]any)
	if !exists {
		return "", fmt.Errorf("architectures field not found in stream data")
	}

	archData, exists := architectures[arch].(map[string]any)
	if !exists {
		return "", fmt.Errorf("architecture %s not found in stream data", arch)
	}

	rhelCoreOSExtensions, exists := archData["rhel-coreos-extensions"].(map[string]any)
	if !exists {
		return "", fmt.Errorf("rhel-coreos-extensions not found for architecture %s", arch)
	}

	azureDisk, exists := rhelCoreOSExtensions["azure-disk"].(map[string]any)
	if !exists {
		return "", fmt.Errorf("azure-disk not found in rhel-coreos-extensions for architecture %s", arch)
	}

	release, exists := azureDisk["release"]
	if !exists {
		return "", fmt.Errorf("release field not found in azure-disk for architecture %s", arch)
	}

	releaseStr, ok := release.(string)
	if !ok {
		return "", fmt.Errorf("release field is not a string in azure-disk for architecture %s", arch)
	}
	return releaseStr, nil
}
