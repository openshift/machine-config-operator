...
func AzureLegacyBootImageTest(oc *exutil.CLI, fixture string) {
	...
	defer func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		framework.Logf("Restored build name in the machineset %s from \"%s\" to \"%s\"", machineSetUnderTest.Name, legacyBootImage, originalBootImage)
	}()
	...
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Arch of MachineSet %s: %s", machineSetUnderTest.Name, arch)

	releaseBootImageVersion, err := getReleaseBootImageVersion(oc, arch)
	o.Expect(err).NotTo(o.HaveOccurred())
	framework.Logf("Current release boot image version: %s", releaseBootImageVersion)

	// TODO: uncomment when the temporary hack for boot image source is removed from reconcileAzureProviderSpec
	// o.Expect(alephVersion).To(o.Equal(releaseBootImageVersion))
	framework.Logf("CoreOS aleph version from new node matches expected boot image: %s", releaseBootImageVersion)
}
