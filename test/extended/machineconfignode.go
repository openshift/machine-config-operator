...
// `ValidateTransitionThroughConditions` validates the condition trasnitions in the MCN during a
// node update. Note that some conditions are passed through quickly in a node update, so the test
// can "miss" catching the phases. For test stability, if we fail to catch an "Unknown" status, a
// warning will be logged instead of erroring out the test.
