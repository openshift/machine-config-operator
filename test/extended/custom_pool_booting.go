...
var _ = g.Describe("[sig-mco][Suite:openshift/machine-config-operator/disruptive][Serial][Disruptive]", func() {
	...
	g.BeforeEach(func() {
		// Skip if machine API is not functional
	...
	g.AfterEach(func(ctx context.Context) {
		...
		deleteMCPErr := machineConfigClient.MachineconfigurationV1().MachineConfigPools().Delete(ctx, customMCPName, metav1.DeleteOptions{})
		if deleteMCPErr != nil {
	...
	g.It("Node booted into custom pool should have appropriate labels and not moved back to worker pool [apigroup:machineconfiguration.openshift.io]", func(ctx context.Context) {
		...
		*clonedMachineSet.Spec.Replicas = 0 // Start with 0 replicas
		...
		logger.Infof("Waiting for new node to be created from machineset %s", clonedMachineSet.Name)
		o.Eventually(func() bool {
			// Get all machines from the cloned machineset
			machines, err := machineClient.MachineV1beta1().Machines(MAPINamespace).List(ctx, metav1.ListOptions{
				LabelSelector: fmt.Sprintf("machine.openshift.io/cluster-api-machineset=%s", clonedMachineSet.Name),
			})
			if err != nil {
			...
			if len(machines.Items) == 0 {
				logger.Infof("No machines found yet for machineset %s", clonedMachineSet.Name)
				return false
			}

			machine := machines.Items[0]
			if machine.Status.NodeRef == nil {
				logger.Infof("Machine %s does not have a node reference yet", machine.Name)
				return false
			}

			// Get the node
			nodeName := machine.Status.NodeRef.Name
			node, err := oc.AsAdmin().KubeClient().CoreV1().Nodes().Get(ctx, nodeName, metav1.GetOptions{})
			if err != nil {
				logger.Infof("Failed to get node %s: %v", nodeName, err)
				return false
			}
			...
			for _, condition := range node.Status.Conditions {
				if condition.Type == corev1.NodeReady && condition.Status == corev1.ConditionTrue {
					...
					logger.Infof("Node %s is ready", nodeName)
					return true
				}
			}

			logger.Infof("Node %s is not ready yet", nodeName)
			return false
		}, 15*time.Minute, 30*time.Second).Should(o.BeTrue(), "Timed out waiting for new node to become ready")
