...
// expectedDisk describes one data disk that the install-config under test is expected to
// have configured. These values are the contract between this test and the CI step that
...
func collectNodeDiskState(oc *exutil.CLI, nodeName, partDevice, mountPath, backingDevice string) (nodeDiskState, error) {
	const sep = "---8<---"
	// The root source is resolved once and reused, so that every fact reported about the
	// root filesystem describes the same device.
	script := fmt.Sprintf(`
root=$(findmnt --first-only --noheadings --output SOURCE --target /)
...
lsblk --nodeps --noheadings --output PKNAME "$1"; echo '%[1]s'
readlink -e "$root"; echo '%[1]s'
lsblk --nodeps --noheadings --output PKNAME "$root"
`, sep)
	...
	if len(sections) != 8 {
		return nodeDiskState{}, fmt.Errorf("expected 8 sections from node %q, got %d in: %s", nodeName, len(sections), out)
	}
	...
	// The MCO debug helper joins stderr onto stdout, so take only the first line of the
	// trailing sections, which is the command's own output.
	state.partitionParent = wholeDevice(firstLine(sections[5]), "")

	// A root filesystem that sits directly on a whole device has no parent name, so fall
	// back to the resolved device itself rather than treating the empty answer as a pass.
	resolvedRoot := strings.TrimSpace(firstLine(sections[6]))
	state.rootParent = wholeDevice(firstLine(sections[7]), resolvedRoot)
	if state.rootParent == "" {
		return nodeDiskState{}, fmt.Errorf("could not resolve the device backing / on node %q from %q", nodeName, state.rootSource)
	}
...
func assertDiskIsMountedAt(ctx context.Context, oc *exutil.CLI, setup diskSetup, mountPath string) {
	...
	for _, node := range getNodesWithRole(ctx, oc, setup.role) {
		...
		o.Eventually(func() error {
			...
			if state.partitionParent == state.rootParent {
				return fmt.Errorf("%s should not live on the same physical device as the root filesystem, both are %q",
					mountPath, state.rootParent)
			}
...
//
// They run only on a cluster installed with the layout in expectedDisks and skip otherwise,
...
var _ = g.Describe("[sig-mco][Suite:openshift/machine-config-operator/parallel][OCPFeatureGate:AzureMultiDisk][OCPFeatureGate:MultiDiskSetup] Azure machine pool disk setup",
	g.Label("Platform:azure"), func() {
		...
		g.BeforeEach(func(ctx context.Context) {
			...
			// between expectedDisks and the job's install-config is silent here. It is not
		...
		g.It("should attach every data disk with the configured storage account type, size and LUN [apigroup:machineconfiguration.openshift.io]", func(ctx context.Context) {
			...
			for _, expected := range expectedDisks {
				...
				// used: the installer labels etcd and swap partitions with the disk setup's
				// type and strips non-alphanumeric characters, so it does not generally equal
				// the data disk's nameSuffix.
