...
func testControlPlaneEtcdCpmsScale(oc *exutil.CLI) {
	...
	for _, node := range nodes {
		...
		if err == nil && cpLabel != "" {
			...
			framework.Logf("Validating etcd disk on control plane node: %s", node.name)
			validateEtcdDiskOnNode(oc, node.name)
...
func validateMultiDiskSetup(oc *exutil.CLI) {
	...
	if foundEtcdDisk && foundSwapDisk {
		...
		for _, node := range nodes {
			cpLabel, err := node.GetLabel("node-role.kubernetes.io/control-plane")
			if err == nil && cpLabel != "" {
				framework.Logf("Validating multi-disk setup on control plane node: %s", node.name)

				// Validate etcd disk
				validateEtcdDiskOnNode(oc, node.name)

				// Check if this node also has swap configured
				nodeObj := NewNode(oc, node.name)
				output, err := nodeObj.DebugNodeWithChroot("swapon", "--show")
				if err == nil && strings.Contains(output, "swap") {
					framework.Logf("Node %s has both etcd and swap disks configured", node.name)
					validateSwapDiskOnNode(oc, node.name)
				}
	...
	if foundSwapDisk && foundUserDefinedDisk {
		...
		for _, node := range nodes {
			...
			if err == nil && workerLabel == "" {
				framework.Logf("Validating multi-disk setup on worker node: %s", node.name)
					...
					for _, line := range lines {
						if strings.Contains(line, "/mnt/") {
							...
							if len(parts) >= 3 {
								...
								validateUserDefinedDiskOnNode(oc, node.name, mountPath)
								break
							}
						}
					}
