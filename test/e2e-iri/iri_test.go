...
// waitForIRICredentialsAccepted polls the IRI registry from the given node
// until it accepts the supplied password. The registry only accepts a password
// once MCD has written the matching htpasswd (02-<role> MachineConfig) to the
// node, so this doubles as a rollout progress check for that MachineConfig.
func waitForIRICredentialsAccepted(ctx context.Context, t *testing.T, cs *framework.ClientSet, node corev1.Node, baseDomain, password string) error {
	t.Helper()
	authHeader := "Basic " + base64.StdEncoding.EncodeToString([]byte(ctrlcommon.IRIRegistryUsername+":"+password))
	return wait.PollUntilContextTimeout(ctx, 10*time.Second, 10*time.Minute, true, func(ctx context.Context) (bool, error) {
		return curlIRIRegistry(t, cs, node, baseDomain, "-H", "Authorization: "+authHeader) == "200", nil
	})
}

// waitForKubeletIRIAuth polls /var/lib/kubelet/config.json on the given node
// until the IRI registry entry carries the credentials for password. That file
// comes from the pull secret (00-<role>) MachineConfig, which is rendered and
// rolled out after the htpasswd (02-<role>) one, so it lags the registry-side
// rotation checked by waitForIRICredentialsAccepted.
func waitForKubeletIRIAuth(ctx context.Context, t *testing.T, cs *framework.ClientSet, node corev1.Node, baseDomain, password string) error {
	t.Helper()
	iriHost := fmt.Sprintf("api-int.%s:%d", baseDomain, ctrlcommon.IRIRegistryPort)
	wantAuth := base64.StdEncoding.EncodeToString([]byte(ctrlcommon.IRIRegistryUsername + ":" + password))
	return wait.PollUntilContextTimeout(ctx, 10*time.Second, 10*time.Minute, true, func(ctx context.Context) (bool, error) {
		...
		return entry["auth"] == wantAuth, nil
	})
}
...
func TestIRIAuth_CredentialRotation(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		t.Logf("Cleanup: restored auth secret, waiting for rollout...")

		// Restoring the secret kicks off the same two sequential rollouts as the
		// rotation itself: 02-master (htpasswd) and then 00-master (pull secret).
		// Wait on the node-observable end state of both before touching the pool
		// condition — right after the secret write the pool is still reporting
		// Updated from the previous rollout, so WaitForPoolCompleteAny on its own
		// would return immediately and leave the node mid-rollout for later tests.
		if err := waitForIRICredentialsAccepted(cleanupCtx, t, cs, node, baseDomain, originalPassword); err != nil {
			t.Errorf("cleanup: original registry credentials were not accepted again: %v", err)
		}
		if err := waitForKubeletIRIAuth(cleanupCtx, t, cs, node, baseDomain, originalPassword); err != nil {
			t.Errorf("cleanup: kubelet config.json was not restored with the original IRI credentials: %v", err)
		}

		// Both MachineConfigs have landed on this node; wait for the pool to
		// finish rolling them out to the remaining masters.
	...
	err = waitForIRICredentialsAccepted(ctx, t, cs, node, baseDomain, newPassword)
	...
	err = waitForKubeletIRIAuth(ctx, t, cs, node, baseDomain, newPassword)
