...
func TestExtensions(t *testing.T) {
	...
	extensions := &mcfgv1.MachineConfig{
		...
		Spec: mcfgv1.MachineConfigSpec{
			...
			Extensions: []string{"ipsec", "usbguard", "kernel-devel", "kerberos"},
		},
	}
	...
	installedPackages := helpers.ExecCmdOnNode(t, cs, node, "chroot", "/rootfs", "rpm", "-q", "libreswan", "usbguard", "kernel-devel", "kernel-headers", "krb5-workstation", "libkadm5")
	expectedPackages := []string{"libreswan", "usbguard", "kernel-devel", "kernel-headers", "krb5-workstation", "libkadm5"}
	...
	installedPackages = helpers.ExecCmdOnNode(t, cs, node, "chroot", "/rootfs", "rpm", "-qa", "libreswan", "usbguard", "kernel-devel", "kernel-headers", "krb5-workstation", "libkadm5")
	for _, v := range expectedPackages {
		if strings.Contains(installedPackages, v) {
			t.Fatalf("Node %s did not rollback successfully", node.Name)
		}
