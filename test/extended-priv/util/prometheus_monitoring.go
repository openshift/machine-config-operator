...
func (mo *Monitor) InstantQueryWithRetry(queryParams MonitorInstantQueryParams, timeDurationSec int) string {
	var res string
	var err error
	err = wait.PollUntilContextTimeout(context.TODO(), time.Duration(timeDurationSec/5)*time.Second, time.Duration(timeDurationSec)*time.Second, true, func(_ context.Context) (bool, error) {
