...
var _ = g.Describe("[sig-mco][Suite:openshift/machine-config-operator/longduration][Serial][Disruptive] MCO security", func() {
	...
	g.It("[PolarionID:65208][OTP] Check the visibility of certificates [Serial]", func() {
		...
		expiryByBundle := map[string]struct {
			Expiry  string
			Subject string
		}{}
		for _, ce := range certsExpiry {
			expiryByBundle[ce.Bundle] = struct {
				Expiry  string
				Subject string
			}{Expiry: ce.Expiry, Subject: ce.Subject}
		}

		for _, certInfo := range ccKCertsInfo {
			ce, ok := expiryByBundle[certInfo.BundleFile]
			o.Expect(ok).To(o.BeTrue(),
				"Missing expiry data for bundle %s", certInfo.BundleFile)
			// Date fields have been temporarily removed by devs:  https://github.com/openshift/machine-config-operator/pull/3866
			o.Expect(ce.Expiry).To(o.Equal(certInfo.NotAfter),
				"Expiry information does not match ControllerConfig")
			o.Expect(ce.Subject).To(o.Equal(certInfo.Subject),
				"Subject information does not match ControllerConfig")
		}
