...
func getBackdatedBootImage(oc *exutil.CLI) string {
	...
	case VspherePlatform:
		...
		o.Expect(
			uploadBaseImageToCloud(oc, platform, url, name),
...
func getBackdatedBootImageNameAndURL(_ *exutil.CLI) (string, string) {
	...
	return "mcotest-" + baseImage, baseImageURL
}

// getReleaseFromVsphereTemplate gets the release version from a vSphere
// template. When server and datacenter are provided, the lookup targets that
// specific vCenter/datacenter; when both are empty it falls back to
// failureDomains[0] for backwards compatibility.
func getReleaseFromVsphereTemplate(oc *exutil.CLI, vsphereTemplate, server, datacenter string) (string, error) {
...
func CheckCurrentOSImageIsUpdated(bir BootImageResource) {
	...
	case VspherePlatform:
		var vsServer, vsDC string
		if ms, ok := bir.(*MachineSet); ok {
			var sErr, dErr error
			vsServer, sErr = ms.Get(`{.spec.template.spec.providerSpec.value.workspace.server}`)
			o.Expect(sErr).NotTo(o.HaveOccurred(), "failed to get workspace.server from MachineSet %s", ms.GetName())
			vsDC, dErr = ms.Get(`{.spec.template.spec.providerSpec.value.workspace.datacenter}`)
			o.Expect(dErr).NotTo(o.HaveOccurred(), "failed to get workspace.datacenter from MachineSet %s", ms.GetName())
		}
		o.Eventually(func() (string, error) {
			...
			return getReleaseFromVsphereTemplate(oc.AsAdmin(), bootImage, vsServer, vsDC)
		}, "5m", "20s").
