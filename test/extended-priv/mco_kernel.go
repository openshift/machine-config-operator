...
var _ = g.Describe("[sig-mco][Suite:openshift/machine-config-operator/longduration][Serial][Disruptive] MCO kernel", func() {
	...
	g.It("[PolarionID:72132][OTP] enable FIPS by Machine-Config-Operator not supported [Disruptive]", func() {
		var (
			...
			mMcName    = "mco-tc-72132-master-fips"
			wMcName    = "mco-tc-72132-worker-fips"
		...
		mMc := NewMachineConfig(oc.AsAdmin(), mMcName, MachineConfigPoolMaster).SetMCOTemplate(mcTemplate)
		...
		wMc := NewMachineConfig(oc.AsAdmin(), wMcName, MachineConfigPoolWorker).SetMCOTemplate(mcTemplate)
	...
	g.It("[PolarionID:72135][OTP] Refuse to disable FIPS mode by Machine-Config-Operator [Disruptive]", func() {
		...
		exutil.By("Try to disable FIPS in worker pool")
