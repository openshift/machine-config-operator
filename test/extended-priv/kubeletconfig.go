...
// waitUntilSuccess polls until the KubeletConfig reports a KubeletConfig has been accepted=True condition.
func (kc KubeletConfig) waitUntilSuccess(timeout string) {
	...
	o.EventuallyWithOffset(1, &kc, timeout, "2s").Should(o.SatisfyAll(
		HaveConditionField("KubeletConfig has been accepted", "status", "True"),
		HaveConditionField("KubeletConfig has been accepted", "message", "Success"),
	), "KubeletConfig '%s' should report KubeletConfig has been accepted=True in status.conditions, but the current status is not success", kc.GetName())
}

// waitUntilFailure polls until the KubeletConfig reports a KubeletConfig has been accepted=False condition with the expected message.
func (kc KubeletConfig) waitUntilFailure(expectedMsg, timeout string) {
	logger.Infof("wait for %s to report failure", kc.name)
	o.EventuallyWithOffset(1, &kc, timeout, "2s").Should(o.SatisfyAll(
		HaveConditionField("KubeletConfig has been accepted", "status", "False"),
		HaveConditionField("KubeletConfig has been accepted", "message", o.ContainSubstring(expectedMsg)),
	), "KubeletConfig '%s' should report KubeletConfig has been accepted=False in status.conditions and report failure message %s. But it doesn't.", kc.GetName(), expectedMsg)
}
