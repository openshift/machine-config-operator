...
func (kc KubeletConfig) waitUntilSuccess(timeout string) {
	...
	o.EventuallyWithOffset(1, &kc, timeout, "2s").Should(o.SatisfyAll(
		HaveConditionField("KubeletConfigIsApplied", "status", "True"),
		HaveConditionField("KubeletConfigIsApplied", "message", "Success"),
...
func (kc KubeletConfig) waitUntilFailure(expectedMsg, timeout string) {
	...
	o.EventuallyWithOffset(1, &kc, timeout, "2s").Should(o.SatisfyAll(
		HaveConditionField("KubeletConfigIsApplied", "status", "False"),
		HaveConditionField("KubeletConfigIsApplied", "message", o.ContainSubstring(expectedMsg)),
	), "KubeletConfig '%s' should report KubeletConfigIsApplied=False in status.conditions and report failure message %s. But it doesn't.", kc.GetName(), expectedMsg)
}
