...
func uploadBaseImageToCloud(ms *MachineSet, platform, baseImageURL, baseImage string) error {
	...
	case VspherePlatform:
		...
		folder, _ := ms.GetWorkspaceFolder()
		...
		g.DeferCleanup(func() {
			logger.Infof("Cleaning up vSphere template %s", baseImage)
			if err := exutil.DeleteVsphereTemplate(baseImage, folder, vsInfo); err != nil {
				logger.Warnf("Failed to delete vSphere template %s: %v", baseImage, err)
			}
		})
