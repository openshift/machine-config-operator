...
func TestMachineOSConfigChangeRestartsBuild(t *testing.T) {
	...
	firstMosb := utils.NewMachineOSBuildFromAPIOrDie(ctx, cs.GetKubeclient(), mosc, mcp)
	...
	moscChangeMosb := utils.NewMachineOSBuildFromAPIOrDie(ctx, cs.GetKubeclient(), apiMosc, mcp)
...
// This test verifie that a change to the MachineConfigPool, such as the
