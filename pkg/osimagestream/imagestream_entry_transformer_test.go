...
func TestImageStreamFileTransformer_FiltersUnneededTags(t *testing.T) {
	...
	entry := &imageutils.InspectionCacheEntry{
		...
		Files:  map[string][]byte{releaseImageStreamPath: encodeImageStream(t, is)},
	}
	...
	resultIS := decodeImageStream(t, result.Files[releaseImageStreamPath])
...
func TestImageStreamFileTransformer_DoesNotMutateInput(t *testing.T) {
	...
	entry := &imageutils.InspectionCacheEntry{
		Files: map[string][]byte{releaseImageStreamPath: originalData},
	}

	transformer := NewImageStreamFileTransformer()
	_ = transformer("sha256:x", entry)

	assert.Equal(t, originalData, entry.Files[releaseImageStreamPath])
}
