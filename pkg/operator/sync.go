...
func (optr *Operator) syncOSImageStreams(_ *renderConfig, _ *configv1.ClusterOperator) error {
	// TODO @pablintino  Perform here a check to see if the OSImageStream resource exist.
	// If it doesn't exist or it's version annotation indicates that it was generated by
	// a different version just create it.
	// Important: This process can be places into a sync.Once and performed only once
	// during the lifetime of the container as the content won't change unless the CVO updates us.
...
func (optr *Operator) syncRenderConfig(_ *renderConfig, _ *configv1.ClusterOperator) error {
	...
	// TODO: @pablintino Replace this logic with a call to get the default stream
	// IMPORTANT: For backguard compatibility purposes, the ControllerConfig fields
	// will still be populated but we will fill them with the default stream.
	// IMPORTANT: Pools should never use these values and should always look the OSImageStream resource
