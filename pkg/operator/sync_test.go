...
func TestSyncMachineConfiguration(t *testing.T) {
	cases := []struct {
		...
		infra                     *configv1.Infrastructure
		expectedManagedBootImages opv1.ManagedBootImages
		annotationExpected        bool
	}{
		{
			name:                      "AWS platform, opt-in expected",
			infra:                     buildInfra(withPlatformType(configv1.AWSPlatformType)),
			mcop:                      buildMachineConfigurationWithBootImageUpdateDisabled(),
			annotationExpected:        true,
			expectedManagedBootImages: getManagedBootImagesWithUpdateEnabled(),
		},
		{
			name:                      "AWS platform, existing config, no opt-in expected",
			infra:                     buildInfra(withPlatformType(configv1.AWSPlatformType)),
		...
		{
			name:                      "GCP platform, opt-in expected",
		...
		{
			name:                      "GCP platform, existing config, no opt-in expected",
			infra:                     buildInfra(withPlatformType(configv1.GCPPlatformType)),
			mcop:                      buildMachineConfigurationWithBootImageUpdateEnabled(),
			annotationExpected:        false,
			expectedManagedBootImages: getManagedBootImagesWithUpdateEnabled(),
		...
		{
			name:                      "Azure platform, no opt-in expected",
			infra:                     buildInfra(withPlatformType(configv1.AzurePlatformType)),
			mcop:                      buildMachineConfigurationWithBootImageUpdateDisabled(),
			annotationExpected:        false,
			expectedManagedBootImages: getManagedBootImagesWithUpdateDisabled(),
		},
		{
			name:                      "vsphere platform, no opt-in expected",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			optr := &Operator{
				...
				fgAccessor: featuregates.NewHardcodedFeatureGateAccess(
					[]configv1.FeatureGateName{features.FeatureGateManagedBootImages, features.FeatureGateNodeDisruptionPolicy}, []configv1.FeatureGateName{},
				),
			...
			assert.Equal(t, tc.annotationExpected, hasAnno)
			assert.Equal(t, mcop.Spec.ManagedBootImages, tc.expectedManagedBootImages)
			klog.Infof("%v", mcop)
		})
	}
}
...
func buildMachineConfigurationWithBootImageUpdateDisabled() *opv1.MachineConfiguration {
	return &opv1.MachineConfiguration{Spec: opv1.MachineConfigurationSpec{ManagedBootImages: getManagedBootImagesWithUpdateDisabled()}, ObjectMeta: metav1.ObjectMeta{Name: "cluster"}}
}

func buildMachineConfigurationWithBootImageUpdateEnabled() *opv1.MachineConfiguration {
	return &opv1.MachineConfiguration{Spec: opv1.MachineConfigurationSpec{ManagedBootImages: getManagedBootImagesWithUpdateEnabled()}, ObjectMeta: metav1.ObjectMeta{Name: "cluster"}}
}
