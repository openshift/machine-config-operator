...
func TestReconcileSimpleContentAccessSecret(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			mcoSecrets:         []*corev1.Secret{},
			expectedMCOSecrets: []corev1.Secret{},
			layeredMCPs:        []*mcfgv1.MachineConfigPool{infraPool},
		},
		{
			name:               "entitlement secret on cluster, with opted-in pool",
			ocManagedSecrets:   []*corev1.Secret{entitlementSecret},
			mcoSecrets:         []*corev1.Secret{},
			layeredMCPs:        []*mcfgv1.MachineConfigPool{infraPool},
			expectedMCOSecrets: []corev1.Secret{*infraEntitlementSecret},
		},
		{
			name:               "entitlement secret on cluster, with multiple opted-in pools",
			ocManagedSecrets:   []*corev1.Secret{entitlementSecret},
			mcoSecrets:         []*corev1.Secret{},
			layeredMCPs:        []*mcfgv1.MachineConfigPool{workerPool, infraPool},
			expectedMCOSecrets: []corev1.Secret{*workerEntitlementSecret, *infraEntitlementSecret},
		},
		{
			name:               "entitlement and cloned secret on cluster, with no opted-in pools",
			ocManagedSecrets:   []*corev1.Secret{entitlementSecret},
			mcoSecrets:         []*corev1.Secret{infraEntitlementSecret},
			layeredMCPs:        []*mcfgv1.MachineConfigPool{},
			expectedMCOSecrets: []corev1.Secret{},
		},
		{
			...
			ocManagedSecrets:   []*corev1.Secret{entitlementSecret},
			mcoSecrets:         []*corev1.Secret{outOfDateInfraEntitlementSecret},
			layeredMCPs:        []*mcfgv1.MachineConfigPool{infraPool},
			expectedMCOSecrets: []corev1.Secret{*infraEntitlementSecret},
		},
	}
