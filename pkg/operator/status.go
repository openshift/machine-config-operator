...
func (optr *Operator) cfeEvalCgroupsV1() (bool, error) {
	...
	nodeClusterConfig, err := optr.nodeClusterLister.Get(ctrlcommonconsts.ClusterNodeInstanceName)
	if err != nil {
...
func isMachineConfigPoolConfigurationValid(fg featuregates.FeatureGate, pool *mcfgv1.MachineConfigPool, version, releaseVersion, osURL string, machineConfigGetter func(string) (*mcfgv1.MachineConfig, error)) error {
	...
	for _, mcName := range mcs {
		...
		v, ok := mc.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey]
	...
	if renderedMC.Spec.OSImageURL != osURL {
		...
		_, ok := renderedMC.Annotations[ctrlcommonconsts.OSImageURLOverriddenKey]
		if !ok {
	...
	rv, ok := renderedMC.Annotations[ctrlcommonconsts.ReleaseImageVersionAnnotationKey]
	...
	if !ok {
		return fmt.Errorf("Unable to access annotation %s for %s expected: %s", ctrlcommonconsts.ReleaseImageVersionAnnotationKey, renderedMC.Name, releaseVersion)
	}
