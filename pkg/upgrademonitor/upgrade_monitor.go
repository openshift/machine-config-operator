...
func GenerateAndApplyMachineConfigNodeSpec(fgAccessor featuregates.FeatureGateAccess, mcpName string, desiredConfigVersion string, node *corev1.Node, mcfgClient mcfgclientset.Interface) error {
	...
	if fg == nil || !fg.Enabled(features.FeatureGateMachineConfigNodes) {
		klog.Infof("MachineConfigNode feature gate is not enabled. Please enable the featureset to use the MCN resource.")
		return nil
	}
