	...
	for i, condition := range newConditions {
		if condition.Type == targetConditionType {
			if condition.Type == opv1.MachineConfigurationBootImageUpdateProgressing {
				...
				// If all machine resources have been reconciled, then the controller is no longer progressing.
				...
				newConditions[i].Message = fmt.Sprintf("%d Degraded MAPI MachineSets | %d Degraded CAPI MachineSets | %d CAPI MachineDeployments | Error(s): %v", ctrl.mapiStats.erroredCount, ctrl.capiMachineSetStats.erroredCount, ctrl.capiMachineDeploymentStats.erroredCount, syncError)
				newConditions[i].Reason = newReason
				// Set reason to BootImageUpdateError if there are any syncerrors. A sync error may happen prior to
				// reconciliation of these resources and may be unrelated to the machine resources.
				if syncError != nil {
					newConditions[i].Status = metav1.ConditionTrue
				} else {
					newConditions[i].Status = metav1.ConditionFalse
...
func getDefaultConditions() []metav1.Condition {
	...
	return []metav1.Condition{
		...
		},
		{
			...
			Message:            "0 Degraded MAPI MachineSets | 0 Degraded CAPI MachineSets | 0 CAPI MachineDeployments | Error(s): ",
			Reason:             "NA",
			LastTransitionTime: metav1.Now(),
			Status:             metav1.ConditionFalse,
		}}
