...
func (ctrl *Controller) updateMAPIMachineSet(oldMS, newMS interface{}) {
	...
	// Only take action if the there is an actual change in the MachineSet Object
...
func (ctrl *Controller) updateMachineConfiguration(oldMC, newMC interface{}) {
	...
	// Only take action if the there is an actual change in the MachineConfiguration API Object
	if oldMachineConfiguration.Generation == newMachineConfiguration.Generation {
		return
	}
...
func (ctrl *Controller) enqueueMAPIMachineSets() error {
	...
	mcop, err := ctrl.mcopLister.Get("cluster")
	if err != nil {
