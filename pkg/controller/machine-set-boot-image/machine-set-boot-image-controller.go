...
func (ctrl *Controller) addConfigMap(obj interface{}) {
	...
	if configMap.Name != ctrlcommonconsts.BootImagesConfigMapName {
		return
	}
...
func (ctrl *Controller) updateConfigMap(oldCM, newCM interface{}) {
	...
	// Take no action if this isn't the "golden" config map
	if oldConfigMap.Name != ctrlcommonconsts.BootImagesConfigMapName {
		return
	}
...
func (ctrl *Controller) addMachineConfiguration(obj interface{}) {
	...
	if machineConfiguration.Name != ctrlcommonconsts.MCOOperatorKnobsObjectName {
		klog.V(4).Infof("MachineConfiguration %s updated, but does not match %s, skipping bootimage sync", machineConfiguration.Name, ctrlcommonconsts.MCOOperatorKnobsObjectName)
		return
	}
...
func (ctrl *Controller) updateMachineConfiguration(oldMC, newMC interface{}) {
	...
	// Take no action if this isn't the "cluster" level MachineConfiguration object
	if oldMachineConfiguration.Name != ctrlcommonconsts.MCOOperatorKnobsObjectName {
		klog.V(4).Infof("MachineConfiguration %s updated, but does not match %s, skipping bootimage sync", oldMachineConfiguration.Name, ctrlcommonconsts.MCOOperatorKnobsObjectName)
		return
	}
...
func (ctrl *Controller) deleteMachineConfiguration(obj interface{}) {
	...
	// Take no action if this isn't the "cluster" level MachineConfiguration object
	if machineConfiguration.Name != ctrlcommonconsts.MCOOperatorKnobsObjectName {
		klog.V(4).Infof("MachineConfiguration %s deleted, but does not match %s, skipping bootimage sync", machineConfiguration.Name, ctrlcommonconsts.MCOOperatorKnobsObjectName)
		return
	}
...
func (ctrl *Controller) syncMAPIMachineSets(reason string) {
	...
	mcop, err := ctrl.mcopLister.Get(ctrlcommonconsts.MCOOperatorKnobsObjectName)
	if err != nil {
...
func (ctrl *Controller) syncMAPIMachineSet(machineSet *machinev1beta1.MachineSet) error {
	...
	if err = wait.PollUntilContextTimeout(context.TODO(), 1*time.Minute, 15*time.Minute, true, func(_ context.Context) (bool, error) {
		...
		configMap, err = ctrl.mcoCmLister.ConfigMaps(ctrlcommonconsts.MCONamespace).Get(ctrlcommonconsts.BootImagesConfigMapName)
		if configMap == nil || err != nil {
			pollError = fmt.Errorf("failed to fetch coreos-bootimages config map during machineset sync: %w", err)
			return false, nil
		}
		versionHashFromCM, versionHashFound := configMap.Data[ctrlcommonconsts.MCOVersionHashKey]
		if !versionHashFound {
			pollError = fmt.Errorf("failed to find mco version hash in %s configmap, sync will exit to wait for the MCO upgrade to complete", ctrlcommonconsts.BootImagesConfigMapName)
			return false, nil
		}
		...
		releaseVersionFromCM, releaseVersionFound := configMap.Data[ctrlcommonconsts.MCOReleaseImageVersionKey]
		if !releaseVersionFound {
			pollError = fmt.Errorf("failed to find mco release version in %s configmap, sync will exit to wait for the MCO upgrade to complete", ctrlcommonconsts.BootImagesConfigMapName)
			return false, nil
		}
...
func (ctrl *Controller) updateConditions(newReason string, syncError error, targetConditionType string) {
	...
	mcop, err := ctrl.mcopClient.OperatorV1().MachineConfigurations().Get(context.TODO(), ctrlcommonconsts.MCOOperatorKnobsObjectName, metav1.GetOptions{})
	if err != nil {
