...
func verifyRegistriesConfigAndPolicyJSONContents(t *testing.T, mc *mcfgv1.MachineConfig, mcName string, imgcfg *apicfgv1.Image, icsps []*apioperatorsv1alpha1.ImageContentSourcePolicy, idmss []*apicfgv1.ImageDigestMirrorSet, itmss []*apicfgv1.ImageTagMirrorSet, clusterImagePolicies []*apicfgv1alpha1.ClusterImagePolicy, releaseImageReg string, verifyPolicyJSON, verifySearchRegsDropin, verifyImagePoliciesRegistriesConfig bool) {
	...
	registriesConf, err := ctrlcommonconfigs.DecodeIgnitionFileContents(regfile.Contents.Source, regfile.Contents.Compression)
	require.NoError(t, err)
	...
	if verifyPolicyJSON {
		...
		policyJSON, err := ctrlcommonconfigs.DecodeIgnitionFileContents(policyfile.Contents.Source, policyfile.Contents.Compression)
		require.NoError(t, err)
	...
	if verifySearchRegsDropin {
		...
		for _, dropinfile := range ignCfg.Storage.Files {
			if dropinfile.Node.Path == searchRegDropInFilePath {
				foundFile = true
				searchRegsConf, err := ctrlcommonconfigs.DecodeIgnitionFileContents(dropinfile.Contents.Source, dropinfile.Contents.Compression)
				require.NoError(t, err)
	...
	if verifyImagePoliciesRegistriesConfig {
		...
		for _, f := range ignCfg.Storage.Files {
			if f.Node.Path == sigstoreRegistriesConfigFilePath {
				...
				registriesYaml, err := ctrlcommonconfigs.DecodeIgnitionFileContents(f.Contents.Source, f.Contents.Compression)
				require.NoError(t, err)
...
func TestContainerRuntimeConfigCreate(t *testing.T) {
	for _, platform := range []apicfgv1.PlatformType{apicfgv1.AWSPlatformType, apicfgv1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			cc := newControllerConfig(ctrlcommonconsts.ControllerConfigName, platform)
			...
			t.Run(string(platform), func(t *testing.T) {
				...
				for i := range pools {
					...
					if tc.clusterImagePolicies == nil {
						...
						verifyRegistriesConfigAndPolicyJSONContents(t, mcs[i], keyReg, imgCfg, tc.icspRules, tc.idmsRules, tc.itmsRules, tc.clusterImagePolicies, cc.Spec.ReleaseImage, true, true, true)
					}
				}
			})
		}
...
func TestContainerruntimeConfigResync(t *testing.T) {
	for _, platform := range []apicfgv1.PlatformType{apicfgv1.AWSPlatformType, apicfgv1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			val := ccr2.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "1", val)

			val = ccr1.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "", val)
...
func TestAddAnnotationExistingContainerRuntimeConfig(t *testing.T) {
	for _, platform := range []apicfgv1.PlatformType{apicfgv1.AWSPlatformType, apicfgv1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			ctrc1.SetAnnotations(map[string]string{ctrlcommonconsts.MCNameSuffixAnnotationKey: "1"})
			...
			_, ok := ctrc.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.False(t, ok)
			val := ctrc1.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "1", val)
			...
			val = ctrc.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "", val)
	...
	for _, platform := range []apicfgv1.PlatformType{apicfgv1.AWSPlatformType, apicfgv1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			ccr1.SetAnnotations(map[string]string{
				ctrlcommonconsts.MCNameSuffixAnnotationKey: "1",
			})
			f.mccrLister = append(f.mccrLister, ccr1)
			f.objects = append(f.objects, ccr1)
			...
			machineConfigDegrade.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey] = versionDegrade
			...
			machineConfigUpgrade.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey] = versionDegrade
			...
			machineConfigDegradeNotGen.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey] = versionDegrade
