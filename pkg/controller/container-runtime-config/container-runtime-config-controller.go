...
func (ctrl *Controller) syncContainerRuntimeConfig(key string) error {
	...
	controllerConfig, err := ctrl.ccLister.Get(ctrlcommonconsts.ControllerConfigName)
	if err != nil {
	...
	for _, pool := range mcpPools {
		...
		if !isNotFound && cfg.Status.ObservedGeneration >= cfg.Generation && cfg.Status.Conditions[len(cfg.Status.Conditions)-1].Type == mcfgv1.ContainerRuntimeConfigSuccess {
			...
			mcCtrlVersion := mc.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey]
		...
		if isNotFound {
			tempIgnCfg := ctrlcommonconfigs.NewIgnConfig()
			mc, err = ctrlcommonconfigs.MachineConfigFromIgnConfig(role, managedKey, tempIgnCfg)
			if err != nil {
				return ctrl.syncStatusOnly(cfg, err, "could not create MachineConfig from new Ignition config: %v", err)
			}
		}
		_, ok := cfg.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
		...
		if _, err := strconv.Atoi(arr[len(arr)-1]); err != nil && !ok {
			if err := ctrl.addAnnotation(cfg, ctrlcommonconsts.MCNameSuffixAnnotationKey, ""); err != nil {
		...
		if len(arr) > 4 && !ok {
			...
			if err == nil {
				if err := ctrl.addAnnotation(cfg, ctrlcommonconsts.MCNameSuffixAnnotationKey, arr[len(arr)-1]); err != nil {
					return ctrl.syncStatusOnly(cfg, err, "could not update annotation for containerRuntimeConfig")
				}
			}
		}
		...
		mc.SetAnnotations(map[string]string{
			ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey: version.Hash,
		})
...
func (ctrl *Controller) cleanUpDuplicatedMC() error {
	...
	for _, mc := range mcList.Items {
		...
		if mc.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey] != version.Hash {
...
func mergeConfigChanges(origFile *ign3types.File, cfg *mcfgv1.ContainerRuntimeConfig, update updateConfigFunc) ([]byte, error) {
	...
	contents, err := ctrlcommonconfigs.DecodeIgnitionFileContents(origFile.Contents.Source, origFile.Contents.Compression)
	if err != nil {
...
func (ctrl *Controller) syncIgnitionConfig(managedKey string, ignFile *ign3types.Config, pool *mcfgv1.MachineConfigPool, ownerRef metav1.OwnerReference) (bool, error) {
	...
	if isNotFound {
		tempIgnCfg := ctrlcommonconfigs.NewIgnConfig()
		mc, err = ctrlcommonconfigs.MachineConfigFromIgnConfig(pool.Name, managedKey, tempIgnCfg)
		if err != nil {
...
func registriesConfigIgnition(templateDir string, controllerConfig *mcfgv1.ControllerConfig, role, releaseImage string,
	...
	if insecureRegs != nil || registriesBlocked != nil || len(icspRules) != 0 || len(idmsRules) != 0 || len(itmsRules) != 0 {
		...
		contents, err := ctrlcommonconfigs.DecodeIgnitionFileContents(originalRegistriesIgn.Contents.Source, originalRegistriesIgn.Contents.Compression)
		if err != nil {
	...
	if policyBlocked != nil || allowedRegs != nil || len(clusterScopePolicies) > 0 {
		...
		contents, err := ctrlcommonconfigs.DecodeIgnitionFileContents(originalPolicyIgn.Contents.Source, originalPolicyIgn.Contents.Compression)
		if err != nil {
...
func RunImageBootstrap(templateDir string, controllerConfig *mcfgv1.ControllerConfig, mcpPools []*mcfgv1.MachineConfigPool, icspRules []*apioperatorsv1alpha1.ImageContentSourcePolicy,
	...
	for _, pool := range mcpPools {
		...
		mc, err := ctrlcommonconfigs.MachineConfigFromIgnConfig(role, managedKey, registriesIgn)
		if err != nil {
			return nil, err
		}
...
func (ctrl *Controller) addFinalizerToContainerRuntimeConfig(ctrCfg *mcfgv1.ContainerRuntimeConfig, mc *mcfgv1.MachineConfig) error {
	return retry.RetryOnConflict(updateBackoff, func() error {
		...
		if !ctrlcommonconfigs.InSlice(mc.Name, ctrCfgTmp.Finalizers) {
