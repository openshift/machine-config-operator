...
func updateRegistriesConfig(data []byte, internalInsecure, internalBlocked []string,
	idmsRules []*apicfgv1.ImageDigestMirrorSet, itmsRules []*apicfgv1.ImageTagMirrorSet) ([]byte, error) {
	tomlConf := sysregistriesv2.V2RegistriesConf{}
	if _, err := toml.Decode(string(data), &tomlConf); err != nil {
		return nil, fmt.Errorf("error unmarshalling registries config: %w", err)
	}

	if err := validateRegistriesConfScopes(internalInsecure, internalBlocked, []string{}, idmsRules, itmsRules); err != nil {
		return nil, err
	}
...
func updatePolicyJSON(data []byte, internalBlocked, internalAllowed []string, releaseImage string) ([]byte, error) {
	...
	if err := validateRegistriesConfScopes([]string{}, internalBlocked, internalAllowed, nil, nil); err != nil {
		return nil, err
	}
...
func getValidBlockedAndAllowedRegistries(releaseImage string, imgSpec *apicfgv1.ImageSpec, idmsRules []*apicfgv1.ImageDigestMirrorSet) (registriesBlocked, policyBlocked, allowed []string, retErr error) {
	if imgSpec == nil {
...
func createTempRegistriesFile(idmsRules []*apicfgv1.ImageDigestMirrorSet, imgSpec *apicfgv1.ImageSpec) (*os.File, error) {
	tomlConf := sysregistriesv2.V2RegistriesConf{}
	if err := registries.EditRegistriesConfig(&tomlConf, imgSpec.RegistrySources.InsecureRegistries, imgSpec.RegistrySources.BlockedRegistries, idmsRules, nil); err != nil {
		return nil, err
	}
	var newData bytes.Buffer
	encoder := toml.NewEncoder(&newData)
...
func validateRegistriesConfScopes(insecure, blocked, allowed []string, idmsRules []*apicfgv1.ImageDigestMirrorSet, itmsRules []*apicfgv1.ImageTagMirrorSet) error {
...
// CreatedICSPLabelKey labels ImageDigestMirrorSet objects that are created from ImageContentSourcePolicy
