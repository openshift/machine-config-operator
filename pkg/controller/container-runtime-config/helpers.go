...
var (
	...
	namespacedPolicyFilePathFormat = filepath.FromSlash("/etc/crio/policies/%s.json")
...
func getManagedKeyNamespacedImagePolicy(pool *mcfgv1.MachineConfigPool) string {
	return fmt.Sprintf("99-%s-generated-imagepolicies", pool.Name)
}
...
func updatePolicyJSON(data []byte, internalBlocked, internalAllowed []string, releaseImage string, clusterScopePolicies map[string]signature.PolicyRequirements) ([]byte, error) {
	...
	updatePolicyObjWithScopeRequirements(policyObj, clusterScopePolicies)
...
func ownerReferenceForImageConfig(imageConfig *apicfgv1.Image) metav1.OwnerReference {
...
func ownerReferenceForImagePolicy(imagePolicies []*apicfgv1alpha1.ImagePolicy, clusterImagePolicies []*apicfgv1alpha1.ClusterImagePolicy) metav1.OwnerReference {
	if len(imagePolicies) > 0 {
		return metav1.OwnerReference{
			APIVersion: apicfgv1alpha1.SchemeGroupVersion.String(),
			Kind:       "ImagePolicy",
			Name:       imagePolicies[0].Name,
			UID:        imagePolicies[0].UID,
		}
	}
	if len(clusterImagePolicies) > 0 {
		return metav1.OwnerReference{
			APIVersion: apicfgv1alpha1.SchemeGroupVersion.String(),
			Kind:       "ClusterImagePolicy",
			Name:       clusterImagePolicies[0].Name,
			UID:        clusterImagePolicies[0].UID,
		}
	}
	return metav1.OwnerReference{}
}
...
func policyItemFromSpec(policy apicfgv1alpha1.Policy) (signature.PolicyRequirement, error) {
	...
	case apicfgv1alpha1.PublicKeyRootOfTrust:
		...
		signedOptions = append(signedOptions, signature.PRSigstoreSignedWithKeyData(keyDataDec))
		if policy.RootOfTrust.PublicKey.RekorKeyData != "" {
			rekorKeyDataDec, err := b64.StdEncoding.DecodeString(policy.RootOfTrust.PublicKey.RekorKeyData)
			if err != nil {
				return nil, fmt.Errorf("failed to decode rekorKeyData: %v", err)
			}
			signedOptions = append(signedOptions, signature.PRSigstoreSignedWithRekorPublicKeyData(rekorKeyDataDec))
		}
	case apicfgv1alpha1.FulcioCAWithRekorRootOfTrust:
		...
		signedOptions = append(signedOptions, signature.PRSigstoreSignedWithFulcio(prSigstoreSignedFulcio))

		rekorKeyDataDec, err := b64.StdEncoding.DecodeString(policy.RootOfTrust.FulcioCAWithRekor.RekorKeyData)
		if err != nil {
			return nil, fmt.Errorf("failed to decode rekorKeyData: %v", err)
		}
		signedOptions = append(signedOptions, signature.PRSigstoreSignedWithRekorPublicKeyData(rekorKeyDataDec))
...
func validateClusterImagePolicyWithAllowedBlockedRegistries(clusterScopePolicies map[string]signature.PolicyRequirements, allowedRegs, policyBlocked []string) error {
	for _, reg := range allowedRegs {
		if _, ok := clusterScopePolicies[reg]; ok {
			return fmt.Errorf("allowedRegistries and clusterimagePolicy configured for the same scope %s ", reg)
		}
	}

	for _, reg := range policyBlocked {
		if _, ok := clusterScopePolicies[reg]; ok {
			return fmt.Errorf("blockedRegistries and clusterimagePolicy configured for the same scope %s ", reg)
		}
	}
	return nil
}

func generateSigstoreRegistriesdConfig(clusterScopePolicies map[string]signature.PolicyRequirements,
	scopeNamespacePolicies map[string]map[string]signature.PolicyRequirements) ([]byte, error) {
	if len(clusterScopePolicies) == 0 && len(scopeNamespacePolicies) == 0 {
		return nil, nil
	}

	dockerConfig := make(map[string]DockerConfig)
	sigstoreAttachment := DockerConfig{
		UseSigstoreAttachments: true,
	}
	for scope := range clusterScopePolicies {
		dockerConfig[scope] = sigstoreAttachment
	}
	...
	registriesConfig.Docker = dockerConfig
...
// updatePolicyObjWithScopeRequirements adds signature requirements to docker transport of the non nil policyObj
func updatePolicyObjWithScopeRequirements(policyObj *signature.Policy, requirements map[string]signature.PolicyRequirements) {
...
// namespacePolicyRequirements collects imagepolicies into map[namespace]map[scope]policyRequirements
func namespaceScopePolicyRequirements(scopeNamespacePolicies map[string]map[string]signature.PolicyRequirements) map[string]map[string]signature.PolicyRequirements {
	policies := make(map[string]map[string]signature.PolicyRequirements)
	for scope, namespacePolicies := range scopeNamespacePolicies {
		for namespace, requirements := range namespacePolicies {
			if policies[namespace] == nil {
				policies[namespace] = make(map[string]signature.PolicyRequirements)
			}
			policies[namespace][scope] = append(policies[namespace][scope], requirements...)
		}
	}
	return policies
}

func generateNamespacedPolicyJSON(namespaceJSONBase []byte, policies map[string]map[string]signature.PolicyRequirements) (map[string][]byte, error) {
	namespaceJSONs := make(map[string][]byte)
	decoder := json.NewDecoder(bytes.NewBuffer(namespaceJSONBase))
	...
	for namespace, requirements := range policies {
		...
		updatePolicyObjWithScopeRequirements(policyObj, requirements)
		data, err := json.Marshal(policyObj)
		if err != nil {
			return nil, fmt.Errorf("error marshalling policy json for namespaced policies: %w", err)
		}
		namespaceJSONs[namespace] = data
	}
...
func imagePolicyConfigFileList(namespaceJSONs map[string][]byte, sigstoreRegistriesConfigYaml []byte) []generatedConfigFile {
	...
	if len(sigstoreRegistriesConfigYaml) > 0 {
		namespacedPolicyConfigFileList = append(namespacedPolicyConfigFileList, generatedConfigFile{
			filePath: sigstoreRegistriesConfigFilePath,
