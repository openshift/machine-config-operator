...
func (ctrl *Controller) processNextWorkItem() bool {
	...
	ctrl.handleQueueErr(ctrl.queue, "containerruntimeconfig", err, key)
...
func (ctrl *Controller) processNextImgWorkItem() bool {
	...
	ctrl.handleQueueErr(ctrl.imgQueue, "image config", err, key)
...
func generateOriginalCredentialProviderConfig(templateDir string, cc *mcfgv1.ControllerConfig, role string) (*ign3types.File, string, error) {
	...
	default:
		credProviderConfigPath = genericCredProviderConfigPath
...
func RunImageBootstrap(templateDir string, controllerConfig *mcfgv1.ControllerConfig, mcpPools []*mcfgv1.MachineConfigPool, icspRules []*apioperatorsv1alpha1.ImageContentSourcePolicy,
	idmsRules []*apicfgv1.ImageDigestMirrorSet, itmsRules []*apicfgv1.ImageTagMirrorSet, imgCfg *apicfgv1.Image, clusterImagePolicies []*apicfgv1.ClusterImagePolicy, imagePolicies []*apicfgv1.ImagePolicy) ([]*mcfgv1.MachineConfig, error) {
...
func crioCredentialProviderConfigIgnition(templateDir string, controllerConfig *mcfgv1.ControllerConfig, role string, crioCredentialProviderConfig *apicfgv1.CRIOCredentialProviderConfig) (*ign3types.Config, []string, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("could not generate original CRIOCredentialProvider config for role %s: %w", role, err)
	}
	if originalCredProviderConfigIgn != nil {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("could not decode CRIOCredentialProvider config for role %s: %w", role, err)
		}
	}
	...
	if len(crioCredentialProviderConfig.Spec.MatchImages) != 0 {
		credProviderConfigYaml, overlappedEntries, err = updateCredentialProviderConfig(credProviderConfigObject, crioCredentialProviderConfig.Spec.MatchImages)
		if err != nil {
			return nil, nil, err
		}

		if credProviderConfigYaml != nil && credProviderConfigPath == genericCredProviderConfigPath {
			CredProviderConfigDropInUnitToml = generateDropinUnitCredProviderConfig(genericCredProviderConfigPath)
		}
	}
	...
	return &credProviderConfigIgn, overlappedEntries, nil
}
