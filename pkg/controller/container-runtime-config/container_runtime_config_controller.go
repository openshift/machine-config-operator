...
func (ctrl *Controller) syncCRIOCredentialProviderConfig(key string) error {
	...
	for _, pool := range mcpPools {
		...
		if err != nil {
			ctrl.syncCRIOCredentialProviderConfigStatusOnly(err, "could not get CRIOCredentialProviderConfig managed key: %v", err)
			return err
		}
		...
		if err := retry.RetryOnConflict(updateBackoff, func() error {
			if crioCredentialProviderConfig != nil {
				...
				credentialProviderConfigIgn, err := crioCredentialProviderConfigIgnition(ctrl.templatesDir, controllerConfig, pool.Name, crioCredentialProviderConfig)
				if err != nil {
					ctrl.syncCRIOCredentialProviderConfigStatusOnly(err, "could not generate CRIOCredentialProvider Ignition config: %v", err)
					return err
				}
				...
				if err != nil {
					ctrl.syncCRIOCredentialProviderConfigStatusOnly(err, "could not sync CRIOCredentialProvider Ignition config: %v", err)
					return err
				}
			}
			...
			ctrl.syncCRIOCredentialProviderConfigStatusOnly(err, "could not Create/Update MachineConfig: %v", err)
			return err
		}
...
func (ctrl *Controller) syncCRIOCredentialProviderConfigStatusOnly(err error, args ...interface{}) {
	statusUpdateErr := retry.RetryOnConflict(updateBackoff, func() error {
		...
		newStatusCondition := wrapErrorWithCRIOCredentialProviderConfigCondition(err, args...)
