...
func (f *fixture) runController(mcpname string, expectError bool) {
	c := f.newController()
	if !c.policyObserversAdded {
		c.addImagePolicyObservers()
		c.policyObserversAdded = true
		c.policyInformerFactoryAdded = true
	}
	if !c.crioCPObserversAdded {
		c.addCRIOCPObservers()
		c.crioCPObserversAdded = true
		c.crioCPInformerFactoryAdded = true
	}
