...
func (ctrl *Controller) syncMachineConfigPool(key string) error {
	...
	if err := AreGeneratedMCsUpdated(mcs); err != nil {
		return err
	}
...
// Ensures the MCs generated by the MCC have the updated controller version hash
func AreGeneratedMCsUpdated(mcs []*mcfgv1.MachineConfig) error {
	for _, mc := range mcs {
		v, ok := mc.Annotations[ctrlcommon.GeneratedByControllerVersionAnnotationKey]
		if ok && v != version.Hash {
			return fmt.Errorf("controller version for %s doesn't match %s has %s", mc.Name, version.Hash, v)
		}
	}
	return nil
}
