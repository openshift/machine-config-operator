...
func (ctrl *Controller) syncGeneratedMachineConfig(pool *mcfgv1.MachineConfigPool, configs []*mcfgv1.MachineConfig) error {
	...
	if generated.Spec.OSImageURL != ctrlcommonconfigs.GetDefaultBaseImageContainer(&cc.Spec) {
	...
	if apierrors.IsNotFound(err) {
		...
		ctrl.eventRecorder.Eventf(pool, corev1.EventTypeNormal, "RenderedConfigGenerated", "%s successfully generated (release version: %s, controller version: %s)",
			generated.Name, generated.Annotations[ctrlcommonconsts.ReleaseImageVersionAnnotationKey], generated.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey])
	}
	if err != nil {
		return err
	}
...
func generateRenderedMachineConfig(pool *mcfgv1.MachineConfigPool, configs []*mcfgv1.MachineConfig, cconfig *mcfgv1.ControllerConfig) (*mcfgv1.MachineConfig, error) {
	...
	for _, config := range configs {
		generatedByControllerVersion := config.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey]
	...
	for _, config := range configs {
		if err := ctrlcommonconfigs.ValidateMachineConfig(config.Spec); err != nil {
			return nil, err
		}
	}

	merged, err := ctrlcommonconfigs.MergeMachineConfigs(configs, cconfig)
	...
	merged.Annotations[ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey] = version.Hash
	merged.Annotations[ctrlcommonconsts.ReleaseImageVersionAnnotationKey] = cconfig.Annotations[ctrlcommonconsts.ReleaseImageVersionAnnotationKey]
	...
	if merged.Spec.OSImageURL != ctrlcommonconfigs.GetDefaultBaseImageContainer(&cconfig.Spec) {
		merged.Annotations[ctrlcommonconsts.OSImageURLOverriddenKey] = "true"
	}
...
func generateAndValidateRenderedMachineConfig(currentMC *mcfgv1.MachineConfig, pool *mcfgv1.MachineConfigPool, configs []*mcfgv1.MachineConfig, cconfig *mcfgv1.ControllerConfig) (*mcfgv1.MachineConfig, error) {
	...
	if err := ctrlcommonconfigs.IsRenderedConfigReconcilable(currentMC, generated); err != nil {
		return nil, goerrs.Join(err, ctrlcommonconfigs.IsComponentConfigsReconcilable(currentMC, configs))
	}
