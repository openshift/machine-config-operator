...
func TestCreatesGeneratedMachineConfig(t *testing.T) {
	...
	cc := newControllerConfig(ctrlcommonconsts.ControllerConfigName)
...
func TestIgnValidationGenerateRenderedMachineConfig(t *testing.T) {
	...
	ignCfg, err := ctrlcommonconfigs.ParseAndConvertConfig(mcs[1].Spec.Config.Raw)
	require.Nil(t, err)
	...
	emptyIgnCfg := ctrlcommonconfigs.NewIgnConfig()
...
func TestGenerateRenderedConfigOnLatestControllerVersionOnly(t *testing.T) {
	mcp := fixtures.NewMachineConfigPool("test-cluster-master", fixtures.MasterSelector, nil, "")
	mcs := []*mcfgv1.MachineConfig{
		fixtures.NewMachineConfigWithAnnotation("00-updated-conf", map[string]string{"node-role/master": ""}, map[string]string{ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey: "2"}, "dummy-test-1", []ign3types.File{}),
		fixtures.NewMachineConfigWithAnnotation("00-old-conf", map[string]string{"node-role/master": ""}, map[string]string{ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey: "1"}, "dummy-change", []ign3types.File{}),
	}
	version.Hash = "2"
	cc := newControllerConfig(ctrlcommonconsts.ControllerConfigName)
	_, err := generateRenderedMachineConfig(mcp, mcs, cc)
	...
	mcs = []*mcfgv1.MachineConfig{
		fixtures.NewMachineConfigWithAnnotation("00-updated-conf", map[string]string{"node-role/master": ""}, map[string]string{ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey: "2"}, "dummy-test-1", []ign3types.File{}),
		fixtures.NewMachineConfigWithAnnotation("99-user-conf", map[string]string{"node-role/master": ""}, map[string]string{ctrlcommonconsts.GeneratedByControllerVersionAnnotationKey: ""}, "user-data", []ign3types.File{}),
	}
	_, err = generateRenderedMachineConfig(mcp, mcs, cc)
	require.Nil(t, err)
}
