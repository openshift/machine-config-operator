...
func (ctrl *Controller) reconcileAuthCredentials(authSecret *corev1.Secret, cconfig *mcfgv1.ControllerConfig) error {
	...
	// Case 1: No IRI entry deployed yet — first-time setup
	if deployedPassword == "" {
	...
	// Case 2: Deployed password matches desired — credentials are in sync.
	...
	// Case 3: Passwords differ — rotation needed
	...
	//
	// Safety: the deployed credentials (oldUser, oldPass) are always preserved
	// as one of the two htpasswd entries. The pull secret (what clients use to
	// authenticate) always contains these old credentials. So even if the desired
	// password changes mid-rotation, clients always authenticate successfully
	// against every htpasswd version.
	...
	if !hasDualEntries {
		// Generate dual htpasswd with both deployed and new credentials so that
		// both passwords are accepted during the rolling MachineConfig update.
	...
	// Dual htpasswd is already set — check if ALL pools have rolled out the
	// dual htpasswd MachineConfig. Only then is it safe to update the pull secret.
	...
	// at which point Case 2 handles cleanup.
	...
	// The pull secret update will trigger the template controller to re-render,
	// creating a new rendered MC. On the next sync, getDeployedIRICredentials will
	// still return the OLD credentials until that rollout completes. Once the new
	// rendered MC (with updated pull secret) is fully deployed, deployedPassword
	// will match desiredPassword, and Case 2 will clean up the dual htpasswd to
	// a single entry.
	return nil
}
