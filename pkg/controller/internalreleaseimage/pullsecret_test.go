...
func TestGenerateDualHtpasswd(t *testing.T) {
	...
	_, err = GenerateDualHtpasswd("openshift", "oldpass", "openshift", "newpass")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "must differ")
}

func TestValidatePullSecretAuthsPreserved(t *testing.T) {
	tests := []struct {
		name            string
		original        string
		merged          string
		iriRegistryHost string
		expectError     bool
		errorContains   string
	}{
		{
			name:            "all entries preserved with IRI added",
			original:        `{"auths":{"quay.io":{"auth":"abc"},"registry.redhat.io":{"auth":"def"}}}`,
			merged:          `{"auths":{"quay.io":{"auth":"abc"},"registry.redhat.io":{"auth":"def"},"api-int.example.com:22625":{"auth":"ghi"}}}`,
			iriRegistryHost: "api-int.example.com:22625",
		},
		{
			name:            "entry removed",
			original:        `{"auths":{"quay.io":{"auth":"abc"},"registry.redhat.io":{"auth":"def"}}}`,
			merged:          `{"auths":{"quay.io":{"auth":"abc"}}}`,
			iriRegistryHost: "api-int.example.com:22625",
			expectError:     true,
			errorContains:   "missing after merge",
		},
		{
			name:            "non-IRI entry value changed",
			original:        `{"auths":{"quay.io":{"auth":"abc"}}}`,
			merged:          `{"auths":{"quay.io":{"auth":"xyz"}}}`,
			iriRegistryHost: "api-int.example.com:22625",
			expectError:     true,
			errorContains:   "was modified during IRI merge",
		},
		{
			name:            "IRI entry value changed is allowed",
			original:        `{"auths":{"quay.io":{"auth":"abc"},"api-int.example.com:22625":{"auth":"old"}}}`,
			merged:          `{"auths":{"quay.io":{"auth":"abc"},"api-int.example.com:22625":{"auth":"new"}}}`,
			iriRegistryHost: "api-int.example.com:22625",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validatePullSecretAuthsPreserved([]byte(tt.original), []byte(tt.merged), tt.iriRegistryHost)
			if tt.expectError {
				assert.Error(t, err)
...
func TestMergeIRIAuthPreservesExistingEntries(t *testing.T) {
	pullSecret := `{"auths":{"quay.io":{"auth":"abc"},"registry.redhat.io":{"auth":"def"},"cloud.openshift.com":{"auth":"ghi"}}}`

	result, err := MergeIRIAuthIntoPullSecret([]byte(pullSecret), "testpassword", "example.com")
	assert.NoError(t, err)
	...
	// All original entries must be preserved with identical values
	origValues := map[string]string{"quay.io": "abc", "registry.redhat.io": "def", "cloud.openshift.com": "ghi"}
	for host, expectedAuth := range origValues {
		entry, ok := auths[host].(map[string]interface{})
		assert.True(t, ok, "original entry %q should be preserved", host)
		assert.Equal(t, expectedAuth, entry["auth"], "original entry %q value should be unchanged", host)
	}

	// IRI entry should be added
	_, ok := auths["api-int.example.com:22625"]
	assert.True(t, ok, "IRI entry should be added")
}
