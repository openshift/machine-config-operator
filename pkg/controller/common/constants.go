...
const (
	...
	// CRIOTLSMinVersionAnnotationKey tracks the TLS min version propagated from KubeletConfig to CRI-O.
	// This is used by the ContainerRuntimeConfig controller to detect when the TLS version changes
	// and force a re-sync even when the ContainerRuntimeConfig generation hasn't changed.
	CRIOTLSMinVersionAnnotationKey = "machineconfiguration.openshift.io/crio-tls-min-version"
