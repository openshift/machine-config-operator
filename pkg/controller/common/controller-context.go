...
func CreateControllerContext(ctx context.Context, cb *clients.Builder) *ControllerContext {
	...
	sharedNamespacedInformers := mcfginformers.NewFilteredSharedInformerFactory(client, resyncPeriod()(), constants.MCONamespace, nil)
	kubeSharedInformer := informers.NewSharedInformerFactory(kubeClient, resyncPeriod()())
	kubeNamespacedSharedInformer := informers.NewFilteredSharedInformerFactory(kubeClient, resyncPeriod()(), constants.MCONamespace, nil)
	...
	apiExtSharedInformer := apiextinformers.NewSharedInformerFactoryWithOptions(apiExtClient, resyncPeriod()(),
		apiextinformers.WithNamespace(constants.MCONamespace), apiextinformers.WithTweakListOptions(assignFilterLabels))
	...
	controllerRef, err := events.GetControllerReferenceForCurrentPod(ctx, kubeClient, constants.MCONamespace, nil)
	if err != nil {
		klog.Warningf("unable to get owner reference (falling back to namespace): %v", err)
	}

	recorder := events.NewKubeRecorder(kubeClient.CoreV1().Events(constants.MCONamespace), "machine-config-operator", controllerRef)
