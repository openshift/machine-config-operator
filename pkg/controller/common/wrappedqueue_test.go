...
func TestWrappedQueue(t *testing.T) {
	...
	wq := NewWrappedQueueForTesting(t, "test-queue")
	...
	enqueueFuncForTest := func(name string, f func(*atomic.Uint64) error) *atomic.Uint64 {
		...
		wq.EnqueueWithName(name, func() error {
			wg.Add(1)
			defer wg.Done()
			counter.Add(1)
			return f(&counter)
	...
	for i := 0; i <= 10; i++ {
		...
		enqueueFuncForTest(name, func(count *atomic.Uint64) error {
			if count.Load() > 1 {
				t.Fatalf("function should only execute once if no error is returned")
			}
	...
	shouldNotPathologicallyFail := enqueueFuncForTest("should not pathologically fail", func(count *atomic.Uint64) error {
		if count.Load() > 1 {
			t.Fatalf("call count for should not pathologcally fail exceeded")
		}

		return fmt.Errorf("i should not pathlogically fail: %w", ErrPathological)
	})
	...
	ctxCancelCount := enqueueFuncForTest("context cancellation", func(count *atomic.Uint64) error {
		if count.Load() > 1 {
			t.Fatalf("call count for should not pathologcally fail exceeded")
		}
	...
	eventually(func() bool {
		return ctxCancelCount.Load() == 1
	})

	eventually(func() bool {
		return shouldNotPathologicallyFail.Load() == 1
	})
