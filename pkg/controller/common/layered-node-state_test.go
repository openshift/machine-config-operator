...
func newNode(current, desired string) *corev1.Node {
	return testfixtures.NewNodeBuilder("").WithCurrentConfig(current).WithDesiredConfig(desired).Node()
}

func newLayeredNode(currentConfig, desiredConfig, currentImage, desiredImage string) *corev1.Node {
	nb := testfixtures.NewNodeBuilder("")
...
func newMachineConfigPool(currentConfig string) *mcfgv1.MachineConfigPool {
	return testfixtures.NewMachineConfigPoolBuilder("").WithMachineConfig(currentConfig).MachineConfigPool()
}

func newLayeredMachineConfigPool(currentConfig string) *mcfgv1.MachineConfigPool {
	return testfixtures.NewMachineConfigPoolBuilder("").WithMachineConfig(currentConfig).WithLayeringEnabled().MachineConfigPool()
}

func newLayeredMachineConfigPoolWithImage(currentConfig, currentImage string) *mcfgv1.MachineConfigPool {
	return testfixtures.NewMachineConfigPoolBuilder("").WithMachineConfig(currentConfig).WithImage(currentImage).MachineConfigPool()
}
...
func TestLayeredNodeState(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			node: testfixtures.NewNodeBuilder("").
