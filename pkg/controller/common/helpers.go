...
//   - Unavailible: The node is either marked unscheduleable or has a MCD actively working. If the
//     MCD is actively working (or hasn't started) then the node *may* go unschedulable in the
//     future, so the node is considered "unavailible" so another node update does not exceed
