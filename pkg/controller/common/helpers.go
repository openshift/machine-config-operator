...
func IgnParseWrapper(rawIgn []byte) (interface{}, error) {
	...
	if errV3 == nil && !rptV3.IsFatal() {
		...
		if errV3_2 != nil {
			...
			return ign3types.Config{}, errors.Errorf("translating Ignition config failed with error %v", errV3_2)
		}
	...
	if errV3.Error() == ign3error.ErrUnknownVersion.Error() {
		...
		if errV2.Error() == ign2error.ErrUnknownVersion.Error() {
			return ign3types.Config{}, errors.Errorf("parsing Ignition config failed: unknown version. Supported spec versions: 2.2, 3.0, 3.1, 3.2, 3.3")
		}
		return ign3types.Config{}, errors.Errorf("parsing Ignition spec v2 failed with error: %v\nReport: %v", errV2, rptV2)
	}

	return ign3types.Config{}, errors.Errorf("parsing Ignition config spec v3 failed with error: %v\nReport: %v", errV3, rptV3)
}
