	...
	return ign3types.File{
		...
		FileEmbedded1: ign3types.FileEmbedded1{
			...
			Contents: ign3types.Resource{
				...
				Compression: coreosutils.StrToPtrStrict(""),
			},
		},
	}
}
...
func NewIgnFileBytesOverwriting(path string, contents []byte) ign3types.File {
	...
	return ign3types.File{
		...
		FileEmbedded1: ign3types.FileEmbedded1{
			...
			Contents: ign3types.Resource{
				Source:      coreosutils.StrToPtr(dataurl.EncodeBytes(contents)),
				Compression: coreosutils.StrToPtrStrict(""), // See https://github.com/coreos/butane/issues/332
			},
		},
	}
}
...
func MergeMachineConfigs(configs []*mcfgv1.MachineConfig, cconfig *mcfgv1.ControllerConfig) (*mcfgv1.MachineConfig, error) {
	...
	for _, config := range configs {
		...
		if config.ObjectMeta.Labels[constants.MachineConfigRoleLabel] == constants.MachineConfigPoolWorker {
	...
	for _, cfg := range configs {
		...
		if cfg.Spec.KernelType == constants.KernelTypeRealtime || cfg.Spec.KernelType == constants.KernelType64kPages {
	...
	if kernelType != constants.KernelTypeDefault {
...
func ValidateMachineConfig(cfg mcfgv1.MachineConfigSpec) error {
	if !(cfg.KernelType == "" || cfg.KernelType == constants.KernelTypeDefault || cfg.KernelType == constants.KernelTypeRealtime || cfg.KernelType == constants.KernelType64kPages) {
