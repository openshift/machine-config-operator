...
func New(
	...
	if ctrl.eventInformer != nil && ctrl.msInformer != nil {
		...
		ctrl.mcInformer.Informer().AddEventHandler(ctrl.eventHandler())
...
func (ctrl *Controller) syncStateController(key string) error {
	...
	*/
	...
	for _, newState := range mcsList {
		...
		nodeRefApplyConfig := machineconfigurationalphav1.MCOObjectReference().WithName(newState.Spec.Node.Name)
		poolRefApplyConfig := machineconfigurationalphav1.MCOObjectReference().WithName(newState.Spec.Pool.Name)
		configVersionApplyConfig := machineconfigurationalphav1.MachineConfigVersion().WithCurrent(newState.Spec.ConfigVersion.Current).WithDesired(newState.Spec.ConfigVersion.Desired)
		statusApplyConfig := machineconfigurationalphav1.MachineConfigNodeStatus().WithConditions(newState.Status.Conditions...).WithObservedGeneration(newState.Generation + 1)
		specApplyConfig := machineconfigurationalphav1.MachineConfigNodeSpec().WithNode(nodeRefApplyConfig).WithPool(poolRefApplyConfig).WithConfigVersion(configVersionApplyConfig)
	...
	for _, newMachineConfiguration := range opMcfgList {
		specApplyConfig := operatorv1.MachineConfigurationSpec().WithMode(newMachineConfiguration.Spec.Mode)
		opApplyCfg := operatorv1.MachineConfigurationComponent().WithConditions(newMachineConfiguration.Status.Operator.Conditions...)
		ctrlApplyCfg := operatorv1.MachineConfigurationComponent().WithConditions(newMachineConfiguration.Status.Controller.Conditions...)
		daemonApplyCfg := operatorv1.MachineConfigurationComponent().WithConditions(newMachineConfiguration.Status.Daemon.Conditions...)
		statusApplyConfig := operatorv1.MachineConfigurationStatus().WithHealth(newMachineConfiguration.Status.Health).WithMostRecentError(newMachineConfiguration.Status.MostRecentError).WithDaemon(daemonApplyCfg).WithOperator(opApplyCfg).WithController(ctrlApplyCfg)
		machineConfigurationApplyConfig := operatorv1.MachineConfiguration(newMachineConfiguration.Name).WithStatus(statusApplyConfig).WithSpec(specApplyConfig)
