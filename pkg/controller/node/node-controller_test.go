...
func TestUpdateCandidates(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			f.ccLister = append(f.ccLister, newControllerConfig(ctrlcommonconsts.ControllerConfigName, configv1.TopologyMode("")))
...
func TestShouldMakeProgress(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.description, func(t *testing.T) {
			...
			f := newFixture(t)
			cc := newControllerConfig(ctrlcommonconsts.ControllerConfigName, configv1.TopologyMode(""))
...
func TestControlPlaneTopology(t *testing.T) {
	t.Parallel()
	f := newFixture(t)
	cc := newControllerConfig(ctrlcommonconsts.ControllerConfigName, configv1.SingleReplicaTopologyMode)
	mcp := fixtures.NewMachineConfigPool("test-cluster-infra", nil, fixtures.InfraSelector, machineConfigV1)
	mcpWorker := fixtures.NewMachineConfigPool("worker", nil, fixtures.WorkerSelector, machineConfigV1)
	mcp.Spec.MaxUnavailable = intStrPtr(intstr.FromInt(1))
