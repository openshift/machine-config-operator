		...
		for _, cond := range mcn.Status.Conditions {
			...
			*/
			...
			if imageModeReportingIsEnabled {
				...
				if mcfgv1.StateProgress(cond.Type) == mcfgv1.MachineConfigNodeUpdated && cond.Status == metav1.ConditionTrue && mcn.Status.ConfigVersion.Desired == pool.Spec.Configuration.Name {
					updatedMachines = append(updatedMachines, ourNode)
					// TODO: understand if node can be degraded in addition to being updated
					break
				}
			}
		}
