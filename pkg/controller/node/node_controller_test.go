...
func newFixture(t *testing.T) *fixture {
	...
	f.fgAccess = featuregates.NewHardcodedFeatureGateAccess(
		[]configv1.FeatureGateName{
			features.FeatureGatePinnedImages, features.FeatureGateOnClusterBuild,
		},
		[]configv1.FeatureGateName{},
	)
	return f
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.mosc != nil {
				...
				//f.moscLister = append(f.moscLister, test.mosc)
			}
			if test.mosb != nil {
...
func TestShouldMakeProgress(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mosb:                  helpers.NewMachineOSBuildBuilder("mosb-1").WithDesiredConfig(machineConfigV1).WithMachineOSConfig("mosc-1").WithSuccessfulBuild().MachineOSBuild(),
			expectAnnotationPatch: true,
			expectTaintsAddPatch:  true,
			buildSuccess:          true,
		},
	...
	for _, test := range tests {
		...
		t.Run(test.description, func(t *testing.T) {
			...
			expStatus := c.calculateStatus(fg, []*mcfgalphav1.MachineConfigNode{}, cc, mcp, nodes, test.mosc, test.mosb)
			expMcp := mcp.DeepCopy()
