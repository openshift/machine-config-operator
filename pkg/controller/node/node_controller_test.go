...
func TestCanLayeredPoolContinue(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			mcp := helpers.NewMachineConfigPool("worker", nil, helpers.WorkerSelector, machineConfigV1)

			reason, canContinue, err := c.canLayeredPoolContinue(mcp, testCase.mosc, testCase.mosb)
			t.Log(reason)
