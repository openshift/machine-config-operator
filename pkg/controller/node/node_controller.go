...
func (ctrl *Controller) syncMachineConfigPool(key string) error {
	...
	for _, node := range nodes {
		...
		layered := ctrlcommon.IsLayeredPool(pool, config, build)
	...
	layered := ctrlcommon.IsLayeredPool(pool, config, build)
	candidates, capacity := getAllCandidateMachines(layered, build, pool, nodes, maxunavail)
	if len(candidates) > 0 {
...
func getAllCandidateMachines(layered bool, build *mcfgv1alpha1.MachineOSBuild, pool *mcfgv1.MachineConfigPool, nodesInPool []*corev1.Node, maxUnavailable int) ([]*corev1.Node, uint) {
...
func getCandidateMachines(pool *mcfgv1.MachineConfigPool, build *mcfgv1alpha1.MachineOSBuild, nodesInPool []*corev1.Node, maxUnavailable int, layered bool) []*corev1.Node {
	nodes, capacity := getAllCandidateMachines(layered, build, pool, nodesInPool, maxUnavailable)
...
func (ctrl *Controller) setDesiredAnnotations(pool *mcfgv1.MachineConfigPool, candidates []*corev1.Node) error {
	...
	if layered := ctrlcommon.IsLayeredPool(pool, config, build); layered {
