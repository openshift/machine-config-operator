...
// Defines the ImagePruner interface, allowing for a fake implementation to be
// used for testing.
...
// Holds the real ImagePruner implementation.
type imagePrunerImpl struct {
	images ImageInspectorDeleter
}

// Constructs a real ImagePruner implementation with a a real ImageInspectorDeleter.
...
// Inspects the given image using the provided secret. Also accepts a
// ControllerConfig so that certificates may be placed on the filesystem.
...
// Deletes the given image using the provided secret. Also accepts a
// ControllerConfig so that certificates may be placed on the filesystem.
...
// Prepares to perform the requested operation by first creating the
// certificate directory, then writing the authfile to the appropriate path.
...
// Cleans up afterward by removing the certs dir and the authfile.
...
// Prepares the certificates by first creating a directory for them and then
// writing the certs to that directory.
...
// Extracts the certificates from the ControllerConfig and writes them to the
// appropriate directory; which defaults to /etc/docker/certs.d
...
// Writes the certificate from the image registry bundle to the certs
...
// Gets the authfile path and writes the secret to the authfile.
...
// Ensures that the image registry secret is in the dockerconfigjson format and
