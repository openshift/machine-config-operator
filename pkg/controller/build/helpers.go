...
func ValidateOnClusterBuildConfig(kubeclient clientset.Interface, mcfgclient versioned.Interface, layeredMCPs []*mcfgv1.MachineConfigPool) error {
	...
	for _, pool := range layeredMCPs {
		...
		if err := validateSecret(kubeclient, moscForPool.Spec.BuildInputs.FinalImagePushSecret.Name); err != nil {
			return err
		}
		if _, err := reference.ParseNamed(moscForPool.Spec.BuildInputs.FinalImagePullspec); err != nil {
			return err
		}

		// Validate the image builder type from the ConfigMap
		if _, err := GetImageBuilderType(moscForPool); err != nil {
			return err
		}
	}
...
func GetImageBuilderType(mosc *mcfgv1alpha1.MachineOSConfig) (ImageBuilderType, error) {
	defaultBuilder := OpenshiftImageBuilder

	if len(mosc.Spec.BuildInputs.ImageBuilderType) == 0 {
		klog.Infof("image builder type not set, defaulting to %q", defaultBuilder)
		return defaultBuilder, nil
	}

	if !validImageBuilderTypes.Has(ImageBuilderType(mosc.Spec.BuildInputs.ImageBuilderType)) {
		return "", &ErrInvalidImageBuilder{Message: fmt.Sprintf("invalid image builder type %q, valid types: %v", mosc.Spec.BuildInputs.ImageBuilderType, validImageBuilderTypes.UnsortedList()), InvalidType: string(mosc.Spec.BuildInputs.ImageBuilderType)}
	}

	klog.Infof("image builder type set to %q", mosc.Spec.BuildInputs.ImageBuilderType)
	return ImageBuilderType(mosc.Spec.BuildInputs.ImageBuilderType), nil
}
