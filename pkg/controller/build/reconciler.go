...
func (b *buildReconciler) reuseImageForNewMOSB(ctx context.Context, mosc *mcfgv1.MachineOSConfig, oldMosb *mcfgv1.MachineOSBuild,
	...
	_, _, inspectErr := daemon.ImageInspect(string(image), authfile)
	if inspectErr != nil {
		klog.V(4).Infof("Image %q no longer exists, rebuilding MOSB %q", image, newMosb.Name)
		_ = b.mcfgclient.MachineconfigurationV1().MachineOSBuilds().Delete(ctx, newMosb.Name, metav1.DeleteOptions{})
		return b.createNewMachineOSBuildForRebuild(ctx, newMosb, mosc.Name)
	}
