...
func (b *buildReconciler) prepareContainerConfig(ctx context.Context, desiredMC *mcfgv1.MachineConfig, owner *mcfgv1.MachineOSConfig) (string, error) {
	...
	containerMC, err := ctrlcommon.GenerateContainerConfig(desiredMC, cc)
	if err != nil {
...
func (b *buildReconciler) prepareContainerConfigForMOSC(ctx context.Context, mosc *mcfgv1.MachineOSConfig) (string, error) {
	...
	return b.prepareContainerConfig(ctx, desMC, mosc)
}
...
func (b *buildReconciler) createNewMachineOSBuildOrReuseExisting(ctx context.Context, mosc *mcfgv1.MachineOSConfig) error {
	...
	mcp, err = b.machineConfigPoolLister.Get(mosc.Spec.MachineConfigPool.Name)
	if err != nil {
		return fmt.Errorf("error getting MachineConfigPool %s: %w",
			mosc.Spec.MachineConfigPool.Name, err)
	}

	containerName, err := b.prepareContainerConfigForMOSC(ctx, mosc)
	if err != nil {
		return err
	}
