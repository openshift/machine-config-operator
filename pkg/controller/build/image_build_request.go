			...
			CommonSpec: buildv1.CommonSpec{
				...
				Output: buildv1.BuildOutput{
					...
					PushSecret: &corev1.LocalObjectReference{
						Name: i.MachineOSConfig.Spec.BuildInputs.FinalImagePushSecret.Name,
					},
					ImageLabels: []buildv1.ImageLabel{
						{Name: "io.openshift.machineconfig.pool", Value: i.MachineOSBuild.Spec.MachineConfigPool.Name},
					},
				},
			},
		},
...
func (i ImageBuildRequest) toPodmanPod() *corev1.Pod {
	env := []corev1.EnvVar{
		...
		{
			Name:  "TAG",
			Value: i.MachineOSConfig.Spec.BuildInputs.FinalImagePullspec,
		},
		{
		...
		Spec: corev1.PodSpec{
			...
			Volumes: []corev1.Volume{ // nolint:dupl // I don't want to deduplicate this yet since there are still some unknowns.
				...
				},
				{
					...
					VolumeSource: corev1.VolumeSource{
						Secret: &corev1.SecretVolumeSource{
							SecretName: i.MachineOSConfig.Spec.BuildInputs.FinalImagePullSecret.Name,
							Items: []corev1.KeyToPath{
								{
									Key:  corev1.DockerConfigJsonKey,
									Path: "config.json",
...
func (i ImageBuildRequest) getObjectMeta(name string) metav1.ObjectMeta {
	return metav1.ObjectMeta{
		...
		Labels: map[string]string{
			...
			targetMachineConfigPoolLabel:    i.MachineOSBuild.Spec.MachineConfigPool.Name,
			desiredConfigLabel:              i.MachineOSBuild.Status.DesiredConfig.Name,
		},
...
func (i ImageBuildRequest) getDockerfileConfigMapName() string {
	return fmt.Sprintf("dockerfile-%s", i.MachineOSBuild.Status.DesiredConfig)
}
...
func (i ImageBuildRequest) getMCConfigMapName() string {
	return fmt.Sprintf("mc-%s", i.MachineOSBuild.Status.DesiredConfig)
}

// Computes the build name based upon the MachineConfigPool name.
func (i ImageBuildRequest) getBuildName() string {
	return fmt.Sprintf("build-%s", i.MachineOSBuild.Status.DesiredConfig)
}

func (i ImageBuildRequest) getDigestConfigMapName() string {
	return fmt.Sprintf("digest-%s", i.MachineOSBuild.Status.DesiredConfig)
}
