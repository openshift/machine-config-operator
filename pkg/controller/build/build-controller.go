...
func newInformers(bcc *Clients) *informers {
	...
	cmInformer := coreinformers.NewFilteredSharedInformerFactory(bcc.kubeclient, 0, ctrlcommonconsts.MCONamespace, nil)
	buildInformer := buildinformers.NewSharedInformerFactoryWithOptions(bcc.buildclient, 0, buildinformers.WithNamespace(ctrlcommonconsts.MCONamespace))
	podInformer := coreinformers.NewSharedInformerFactoryWithOptions(bcc.kubeclient, 0, coreinformers.WithNamespace(ctrlcommonconsts.MCONamespace))
...
func (ctrl *Controller) postBuildCleanup(mosc *mcfgv1alpha1.MachineOSConfig, mosb *mcfgv1alpha1.MachineOSBuild, ignoreMissing bool) error {
	...
	deleteMCConfigMap := func() error {
		...
		err := ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Delete(context.TODO(), ibr.getMCConfigMapName(), metav1.DeleteOptions{})
	...
	deleteDockerfileConfigMap := func() error {
		...
		err := ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Delete(context.TODO(), ibr.getDockerfileConfigMapName(), metav1.DeleteOptions{})
...
func (ctrl *Controller) markBuildSucceeded(mosc *mcfgv1alpha1.MachineOSConfig, mosb *mcfgv1alpha1.MachineOSBuild) error {
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		ibr := newImageBuildRequest(mosc, mosb)
		digestConfigMap, err := ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Get(context.TODO(), ibr.getDigestConfigMapName(), metav1.GetOptions{})
		if err != nil {
			return err
		}
...
func (ctrl *Controller) getOSImageURLConfig() (*ctrlcommon.OSImageURLConfig, error) {
	cm, err := ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Get(context.TODO(), ctrlcommonconsts.MachineConfigOSImageURLConfigMapName, metav1.GetOptions{})
	if err != nil {
		return nil, fmt.Errorf("could not get ConfigMap %s: %w", ctrlcommonconsts.MachineConfigOSImageURLConfigMapName, err)
	}
...
func (ctrl *Controller) getImagesConfig() (*ctrlcommon.Images, error) {
	cm, err := ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Get(context.TODO(), ctrlcommonconsts.MachineConfigOperatorImagesConfigMapName, metav1.GetOptions{})
	if err != nil {
		return nil, fmt.Errorf("could not get configmap %s: %w", ctrlcommonconsts.MachineConfigOperatorImagesConfigMapName, err)
	}
...
func (ctrl *Controller) prepareForBuild(mosb *mcfgv1alpha1.MachineOSBuild, mosc *mcfgv1alpha1.MachineOSConfig) (ImageBuildRequest, error) {
	...
	_, err = ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Create(context.TODO(), mcConfigMap, metav1.CreateOptions{})
	if err != nil {
	...
	_, err = ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Create(context.TODO(), dockerfileConfigMap, metav1.CreateOptions{})
	if err != nil {
...
func (ctrl *Controller) getOptionalSecret(secretName string) (*corev1.Secret, error) {
	optionalSecret, err := ctrl.kubeclient.CoreV1().Secrets(ctrlcommonconsts.MCONamespace).Get(context.TODO(), secretName, metav1.GetOptions{})
	if err == nil {
...
func (ctrl *Controller) getOptionalConfigMap(configmapName string) (*corev1.ConfigMap, error) {
	optionalConfigMap, err := ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Get(context.TODO(), configmapName, metav1.GetOptions{})
	if err == nil {
...
func (ctrl *Controller) validatePullSecret(name string) (*corev1.Secret, error) {
	secret, err := ctrl.kubeclient.CoreV1().Secrets(ctrlcommonconsts.MCONamespace).Get(context.TODO(), name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (ctrl *Controller) handleCanonicalizedPullSecret(secret *corev1.Secret) (*corev1.Secret, error) {
	...
	out, err := ctrl.kubeclient.CoreV1().Secrets(ctrlcommonconsts.MCONamespace).Get(context.TODO(), secret.Name, metav1.GetOptions{})
...
func (ctrl *Controller) createCanonicalizedSecret(secret *corev1.Secret) (*corev1.Secret, error) {
	...
	out, err := ctrl.kubeclient.CoreV1().Secrets(ctrlcommonconsts.MCONamespace).Create(context.TODO(), secret, metav1.CreateOptions{})
...
func (ctrl *Controller) updateCanonicalizedSecret(secret *corev1.Secret) (*corev1.Secret, error) {
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		found, err := ctrl.kubeclient.CoreV1().Secrets(ctrlcommonconsts.MCONamespace).Get(context.TODO(), secret.Name, metav1.GetOptions{})
		if err != nil && !k8serrors.IsNotFound(err) {
			return fmt.Errorf("could not get canonical secret %q: %w", secret.Name, err)
		}

		found.Data = secret.Data
		updated, err := ctrl.kubeclient.CoreV1().Secrets(ctrlcommonconsts.MCONamespace).Update(context.TODO(), found, metav1.UpdateOptions{})
		if err != nil {
