...
func (ctrl *PodBuildController) StartBuild(ibr ImageBuildRequest) (*corev1.ObjectReference, error) {
	...
	if pod != nil && err == nil && hasAllRequiredOSBuildLabels(pod.Labels) {
		klog.Infof("Found preexisting build pod (%s) for pool %s", pod.Name, ibr.MachineOSBuild.Spec.MachineConfigPool.Name)
	...
	klog.Infof("Build started for pool %s in %s!", ibr.MachineOSBuild.Spec.MachineConfigPool.Name, pod.Name)
