...
func (ctrl *PodBuildController) syncPod(key string) error { //nolint:dupl // This does have commonality with the ImageBuildController.
	...
	pod, err := ctrl.podLister.Pods(ctrlcommonconsts.MCONamespace).Get(name)
	...
	pod, err = ctrl.kubeclient.CoreV1().Pods(ctrlcommonconsts.MCONamespace).Get(context.TODO(), pod.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
...
func (ctrl *PodBuildController) DeleteBuildObject(mosb *mcfgv1alpha1.MachineOSBuild, mosc *mcfgv1alpha1.MachineOSConfig) error {
	...
	return aggerrors.AggregateGoroutines(
		func() error {
			...
			return ignoreIsNotFoundErr(ctrl.kubeclient.CoreV1().Pods(ctrlcommonconsts.MCONamespace).Delete(context.TODO(), ibr.getBuildName(), metav1.DeleteOptions{}))
		},
		func() error {
			ibr := newImageBuildRequest(mosc, mosb)
			return ignoreIsNotFoundErr(ctrl.kubeclient.CoreV1().ConfigMaps(ctrlcommonconsts.MCONamespace).Delete(context.TODO(), ibr.getDigestConfigMapName(), metav1.DeleteOptions{}))
		},
	)
}
...
func (ctrl *PodBuildController) IsBuildRunning(mosb *mcfgv1alpha1.MachineOSBuild, mosc *mcfgv1alpha1.MachineOSConfig) (bool, error) {
	...
	_, err := ctrl.kubeclient.CoreV1().Pods(ctrlcommonconsts.MCONamespace).Get(context.TODO(), ibr.getBuildName(), metav1.GetOptions{})
...
func (ctrl *PodBuildController) StartBuild(ibr ImageBuildRequest) (*corev1.ObjectReference, error) {
	...
	// First check if we have a build in progress for this MachineConfigPool and rendered config.
	pod, err := ctrl.kubeclient.CoreV1().Pods(ctrlcommonconsts.MCONamespace).Get(context.TODO(), ibr.getBuildName(), metav1.GetOptions{})
	if err != nil && !k8serrors.IsNotFound(err) {
		return nil, err
	}
	...
	pod, err = ctrl.kubeclient.CoreV1().Pods(ctrlcommonconsts.MCONamespace).Create(context.TODO(), ibr.toBuildPod(), metav1.CreateOptions{})
	if err != nil {
