...
func createNewVMTemplate(streamData *stream.Stream, providerSpec *machinev1beta1.VSphereMachineProviderSpec, infra *osconfigv1.Infrastructure, credsSc *corev1.Secret, kubeClient clientset.Interface, arch, release string) (string, bool, error) {
	...
	for _, vcenter := range infra.Spec.PlatformSpec.VSphere.VCenters {
		...
		for _, failureDomain := range infra.Spec.PlatformSpec.VSphere.FailureDomains {
			...
			return reconcileTemplateVersion(ctx, finder, providerSpec, failureDomain, streamData, client, tagManager, existingTemplateVM, vmMo, computedName, resolvedName, infraID, arch, release, templateProductVersion)
		}
	}
...
func reconcileTemplateVersion(ctx context.Context, finder *find.Finder, providerSpec *machinev1beta1.VSphereMachineProviderSpec, failureDomain osconfigv1.VSpherePlatformFailureDomainSpec, streamData *stream.Stream, client *govmomi.Client, tagManager *tags.Manager, existingTemplateVM *object.VirtualMachine, vmMo mo.VirtualMachine, computedName, resolvedName, infraID, arch, release, templateProductVersion string) (string, bool, error) {
