	...
	for _, info := range infos {
		...
		for idx, m := range manifests {
			...
			case *apicfgv1.FeatureGate:
				if obj.GetName() == ctrlcommonconsts.ClusterFeatureInstanceName {
					featureGate = obj
				}
			case *apicfgv1.Node:
				if obj.GetName() == ctrlcommonconsts.ClusterNodeInstanceName {
					nodeConfig = obj
				}
			case *apicfgv1.APIServer:
				if obj.GetName() == ctrlcommonconsts.APIServerInstanceName {
					apiServer = obj
				}
			default:
