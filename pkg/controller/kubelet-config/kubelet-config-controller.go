...
func (ctrl *Controller) filterAPIServer(apiServer *configv1.APIServer) {
	...
	if nodeConfig, err := ctrl.configClient.ConfigV1().Nodes().Get(context.TODO(), ctrlcommonconsts.ClusterNodeInstanceName, metav1.GetOptions{}); err != nil {
...
func generateOriginalKubeletConfigWithFeatureGates(cc *mcfgv1.ControllerConfig, templatesDir, role string, featureGateAccess featuregates.FeatureGateAccess, apiServer *configv1.APIServer) (*kubeletconfigv1beta1.KubeletConfiguration, error) {
	...
	contents, err := ctrlcommonconfigs.DecodeIgnitionFileContents(originalKubeletIgn.Contents.Source, originalKubeletIgn.Contents.Compression)
	if err != nil {
...
func (ctrl *Controller) syncKubeletConfig(key string) error {
	...
	if err := apihelpers.IsControllerConfigCompleted(ctrlcommonconsts.ControllerConfigName, ctrl.ccLister.Get); err != nil {
		return err
	}
	...
	apiServer, err := ctrl.apiserverLister.Get(ctrlcommonconsts.APIServerInstanceName)
	if err != nil && !macherrors.IsNotFound(err) {
		return ctrl.syncStatusOnly(cfg, err, "could not get the TLSSecurityProfile from %v: %v", ctrlcommonconsts.APIServerInstanceName, err)
	}
	...
	for _, pool := range mcpPools {
		...
		if role == ctrlcommonconsts.MachineConfigPoolWorker {
		...
		if isNotFound {
			...
			mc, err = ctrlcommonconfigs.MachineConfigFromIgnConfig(role, managedKey, ignConfig)
			if err != nil {
...
func (ctrl *Controller) addFinalizerToKubeletConfig(kc *mcfgv1.KubeletConfig, mc *mcfgv1.MachineConfig) error {
	return retry.RetryOnConflict(updateBackoff, func() error {
		...
		if !ctrlcommonconfigs.InSlice(mc.Name, kcTmp.ObjectMeta.Finalizers) {
