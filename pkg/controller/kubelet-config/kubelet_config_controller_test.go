...
// TestKubeletConfigTLSRender verifies that template rendering produces the correct TLS drop-in
// for each security profile type (Old, Intermediate, Custom, nil/empty).
func TestKubeletConfigTLSRender(t *testing.T) {
	...
	for _, testCase := range TestCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			tlsDropIn, err := renderAndExtractFile(cc, ctrl.templatesDir, "master", testCase.apiserver, constants.KubeletTLSDropInPath)
			require.NoError(t, err, "could not generate TLS drop-in from templates")
			require.NotNil(t, tlsDropIn, "TLS drop-in file should not be nil")

			contents, err := ctrlcommon.DecodeIgnitionFileContents(tlsDropIn.Contents.Source, tlsDropIn.Contents.Compression)
			require.NoError(t, err)
			tlsKubeConfig, err := DecodeKubeletConfig(contents)
			require.NoError(t, err)
			...
			require.Equal(t, tlsKubeConfig.TLSCipherSuites, expectedTLSCipherSuites)
			require.Equal(t, tlsKubeConfig.TLSMinVersion, expectedTLSMinVersion)
		})
	}
}

// TestKubeletConfigTLSOverride verifies that KubeletConfig.spec.tlsSecurityProfile
// takes precedence over APIServer.spec.tlsSecurityProfile in the TLS drop-in.
func TestKubeletConfigTLSOverride(t *testing.T) {
	...
	// Grab the MachineConfig generated by the controller, and decode the TLS drop-in file
	...
	require.NoError(t, err)
	var tlsDropInFile ign3types.File
	for _, file := range generatedIgnConfig.Storage.Files {
		if file.Path == constants.KubeletTLSDropInPath {
			tlsDropInFile = file
		}
	}
	require.NotEmpty(t, tlsDropInFile, "TLS drop-in file should be present in generated MachineConfig")

	// Decode to a native KubeletConfiguration object
	contents, err := ctrlcommon.DecodeIgnitionFileContents(tlsDropInFile.Contents.Source, tlsDropInFile.Contents.Compression)
	require.NoError(t, err)
