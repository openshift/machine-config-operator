...
func (ctrl *Controller) syncStatusOnly(cfg *mcfgv1.KubeletConfig, err error, args ...interface{}) error {
	statusUpdateError := retry.RetryOnConflict(updateBackoff, func() error {
		...
		// TODO(OCPNODE-4040): Remove the next line once openshift/origin and openshift/oc use KubeletConfigAccepted.
...
func (ctrl *Controller) syncKubeletConfig(key string) error {
	...
	for _, pool := range mcpPools {
		...
		kubeletIgnition, logLevelIgnition, autoSizingReservedIgnition, systemGomaxprocsIgnition, err := generateKubeletIgnFiles(cfg, originalKubeConfig, systemGomaxprocsBehavior)
		if err != nil {
			return ctrl.syncStatusOnly(cfg, err)
		}
		...
		if systemGomaxprocsIgnition != nil {
			tempIgnConfig.Storage.Files = append(tempIgnConfig.Storage.Files, *systemGomaxprocsIgnition)
		}
