...
func (ctrl *Controller) syncNodeConfigHandler(key string) error {
	...
	if err != nil && !errors.IsNotFound(err) {
		return fmt.Errorf("could not get the TLSSecurityProfile from %v: %v", ctrlcommonconsts.APIServerInstanceName, err)
	}
	...
	for _, pool := range mcpPools {
		...
		if isNotFound {
			...
			mc, err = ctrlcommonconfigs.MachineConfigFromIgnConfig(role, key, ignConfig)
			if err != nil {
				return err
			}
		}
		...
		if role == ctrlcommonconsts.MachineConfigPoolWorker || role == ctrlcommonconsts.MachineConfigPoolMaster {
	...
	features, err := ctrl.featLister.Get(ctrlcommonconsts.ClusterFeatureInstanceName)
	if err == nil {
...
func (ctrl *Controller) updateNodeConfig(old, cur interface{}) {
	...
	if newNode.Name != ctrlcommonconsts.ClusterNodeInstanceName {
		message := fmt.Sprintf("The node.config.openshift.io \"%v\" is invalid: metadata.name Invalid value: \"%v\" : must be \"%v\"", newNode.Name, newNode.Name, ctrlcommonconsts.ClusterNodeInstanceName)
...
func (ctrl *Controller) addNodeConfig(obj interface{}) {
	...
	if nodeConfig.Name != ctrlcommonconsts.ClusterNodeInstanceName {
		message := fmt.Sprintf("The node.config.openshift.io \"%v\" is invalid: metadata.name Invalid value: \"%v\" : must be \"%v\"", nodeConfig.Name, nodeConfig.Name, ctrlcommonconsts.ClusterNodeInstanceName)
		klog.V(2).Infof(message)
