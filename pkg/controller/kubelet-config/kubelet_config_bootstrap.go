...
func RunKubeletBootstrap(ctx context.Context, templateDir string, kubeletConfigs []*mcfgv1.KubeletConfig, controllerConfig *mcfgv1.ControllerConfig, features *configv1.FeatureGate, nodeConfig *configv1.Node, mcpPools []*mcfgv1.MachineConfigPool) ([]*mcfgv1.MachineConfig, error) {
	...
	for _, kubeletConfig := range kubeletConfigs {
		...
		for _, pool := range mcpPools {
			...
			managedKey, err := generateBootstrapManagedKeyKubelet(ctx, pool, managedKeyExist)
			if err != nil {
				return nil, err
			}
...
func generateBootstrapManagedKeyKubelet(ctx context.Context, pool *mcfgv1.MachineConfigPool, managedKeyExist map[string]bool) (string, error) {
	...
	managedKey, err := ctrlcommon.GetManagedKey(ctx, pool, nil, "99", "kubelet", "")
	if err != nil {
		return "", err
	}
