...
func generateKubeletIgnFiles(kubeletConfig *mcfgv1.KubeletConfig, originalKubeConfig *kubeletconfigv1beta1.KubeletConfiguration, systemGomaxprocsBehavior mcfgv1.GomaxprocsBehaviorType) (*ign3types.File, *ign3types.File, *ign3types.File, error) {
	...
	autoSizingReservedIgnition = createAutoSizingReservedIgnition(kubeletConfig.Spec.AutoSizingReserved, userDefinedSystemReserved, systemGomaxprocsBehavior)

	return kubeletIgnition, logLevelIgnition, autoSizingReservedIgnition, nil
}

// createAutoSizingReservedIgnition builds the /etc/node-sizing-enabled.env ignition file when any of
// auto-sizing, user-defined system reserved values, or a system GOMAXPROCS behavior is configured.
// It returns nil when none of those are set.
func createAutoSizingReservedIgnition(autoSizingReserved *bool, userDefinedSystemReserved map[string]string, systemGomaxprocsBehavior mcfgv1.GomaxprocsBehaviorType) *ign3types.File {
	if len(userDefinedSystemReserved) > 0 {
		return createNewKubeletDynamicSystemReservedIgnition(nil, userDefinedSystemReserved, systemGomaxprocsBehavior)
	}
	if autoSizingReserved != nil {
		return createNewKubeletDynamicSystemReservedIgnition(autoSizingReserved, userDefinedSystemReserved, systemGomaxprocsBehavior)
	}
	if systemGomaxprocsBehavior != "" {
		return createNewKubeletDynamicSystemReservedIgnition(nil, userDefinedSystemReserved, systemGomaxprocsBehavior)
	}
	return nil
}
