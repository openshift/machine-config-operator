	...
	f.apiserverLister = []*osev1.APIServer{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name: ctrlcommonconsts.APIServerInstanceName,
			},
		},
	}
	return f
...
func TestKubeletConfigResync(t *testing.T) {
	for _, platform := range []osev1.PlatformType{osev1.AWSPlatformType, osev1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			val := kc2.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "1", val)

			val = kc1.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "", val)
...
func TestAddAnnotationExistingKubeletConfig(t *testing.T) {
	for _, platform := range []osev1.PlatformType{osev1.AWSPlatformType, osev1.NonePlatformType, "unrecognized"} {
		t.Run(string(platform), func(t *testing.T) {
			...
			kc1.SetAnnotations(map[string]string{ctrlcommonconsts.MCNameSuffixAnnotationKey: "1"})
			...
			_, ok := kc.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.False(t, ok)
			val := kc1.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "1", val)
			...
			val = kc.GetAnnotations()[ctrlcommonconsts.MCNameSuffixAnnotationKey]
			require.Equal(t, "", val)
...
func TestKubeletConfigTLSRender(t *testing.T) {
	TestCases := []struct {
	...
	}{
		{
			...
			apiserver: &osev1.APIServer{ObjectMeta: metav1.ObjectMeta{Name: ctrlcommonconsts.APIServerInstanceName}},
		},
		{
		...
		},
		{
			...
			apiserver: &osev1.APIServer{
				ObjectMeta: metav1.ObjectMeta{Name: ctrlcommonconsts.APIServerInstanceName},
	...
	for _, testCase := range TestCases {
		t.Run(testCase.name, func(t *testing.T) {
			t.Parallel()
			f := newFixture(t)
			cc := newControllerConfig(ctrlcommonconsts.ControllerConfigName, osev1.AWSPlatformType)
			f.ccLister = append(f.ccLister, cc)
			...
			contents, err := ctrlcommonconfigs.DecodeIgnitionFileContents(kubeletConfig.Contents.Source, kubeletConfig.Contents.Compression)
			require.NoError(t, err)
...
func TestKubeletConfigTLSOverride(t *testing.T) {
	...
	generatedIgnConfig, err := ctrlcommonconfigs.ParseAndConvertConfig(generatedConfig.Spec.Config.Raw)
	require.NoError(t, err)
	...
	contents, err := ctrlcommonconfigs.DecodeIgnitionFileContents(kubeletConfigFile.Contents.Source, kubeletConfigFile.Contents.Compression)
	require.NoError(t, err)
