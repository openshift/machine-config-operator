...
func (ctrl *Controller) syncFeatureHandler(key string) error {
	...
	for _, pool := range mcpPools {
		...
		rawCfgIgn, err := generateKubeConfigIgnFromFeatures(
			...
			ctrl.fgHandler,
			nodeConfig,
			apiServer, osimagestream.GetOSImageStreamFromPoolListByPoolNameDefault(osImageStream, mcpPools, role, nil),
		)
		if err != nil {
			return err
		}
...
func RunFeatureGateBootstrap(templateDir string, fgHandler ctrlcommon.FeatureGatesHandler, nodeConfig *osev1.Node, controllerConfig *mcfgv1.ControllerConfig, mcpPools []*mcfgv1.MachineConfigPool, apiServer *osev1.APIServer, osImageStream *v1alpha1.OSImageStream) ([]*mcfgv1.MachineConfig, error) {
	...
	for _, pool := range mcpPools {
		...
		rawCfgIgn, err := generateKubeConfigIgnFromFeatures(
			...
			nodeConfig,
			apiServer,
			osimagestream.GetOSImageStreamFromPoolListByPoolNameDefault(osImageStream, mcpPools, role, nil),
		)
		if err != nil {
			return nil, err
		}
