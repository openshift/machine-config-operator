...
func TestWrapErrorWithCondition(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "error without args produces KubeletConfigIsApplied condition with status False",
			err:             fmt.Errorf("KubeletConfiguration: swapBehavior is not allowed to be set, but contains: LimitedSwap"),
			args:            nil,
			expectedType:    mcfgv1.KubeletConfigApplied,
			expectedStatus:  corev1.ConditionFalse,
			expectedMessage: "Error: KubeletConfiguration: swapBehavior is not allowed to be set, but contains: LimitedSwap",
		},
		{
			name:            "error with formatted args produces KubeletConfigIsApplied condition with status False",
		...
		{
			name:            "nil error produces KubeletConfigIsApplied condition with status True",
			err:             nil,
			args:            nil,
			expectedType:    mcfgv1.KubeletConfigApplied,
		...
		{
			name:            "nil error with args still produces KubeletConfigIsApplied condition with status True",
			err:             nil,
