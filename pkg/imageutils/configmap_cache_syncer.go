...
func (s *ConfigMapCacheSyncer) Load(ctx context.Context) (map[string]*InspectionCacheEntry, error) {
...
func (s *ConfigMapCacheSyncer) save(ctx context.Context, entries map[string]*InspectionCacheEntry) error {
	...
	cm := &corev1.ConfigMap{
		...
		Data: map[string]string{
			cacheConfigMapKey: string(data),
		},
	}

	_, err = s.kubeclient.CoreV1().ConfigMaps(s.namespace).Create(ctx, cm, metav1.CreateOptions{})
	if err == nil {
	...
	if err != nil {
		return fmt.Errorf("getting existing inspection cache ConfigMap: %w", err)
	}
	existing.Data = cm.Data
	_, err = s.kubeclient.CoreV1().ConfigMaps(s.namespace).Update(ctx, existing, metav1.UpdateOptions{})
	if err != nil {
