...
func (s *SysContextPaths) newControllerConfig() (*mcfgv1.ControllerConfig, error) {
	...
	// Load additional trust bundles if provided
	if len(s.AdditionalTrustBundles) > 0 {
	...
	// Load per-host certificates if provided
	if s.PerHostCertDir != "" {
...
// Only accepts CA certificate files with extensions: .crt, .pem, .cert, .cer
// Preserves the host-relative directory structure by using relative paths.
func (s *SysContextPaths) loadPerHostCertificates() ([]mcfgv1.ImageRegistryBundle, error) {
	...
	err := filepath.Walk(s.PerHostCertDir, func(path string, info fs.FileInfo, err error) error {
		...
		// Only accept CA certificate files by extension
		...
		// Preserve host-relative path instead of just the base name
		relPath, err := filepath.Rel(s.PerHostCertDir, path)
		if err != nil {
			return fmt.Errorf("could not compute relative path for %q: %w", path, err)
		}

		certs = append(certs, mcfgv1.ImageRegistryBundle{Data: fileBytes, File: relPath})
