...
type mockSystemdConnection struct {
	...
	// presetPolicies maps unit names to their preset outcome (true=enable, false=disable).
	// Units not in the map default to false (disable).
	presetPolicies map[string]bool
...
func newMockSystemdConnection(units map[string]*mockUnitState) *mockSystemdConnection {
	...
	return &mockSystemdConnection{
		units:          units,
		presetPolicies: make(map[string]bool),
	}
}
...
func (m *mockSystemdConnection) Enable(ctx context.Context, force bool, units ...string) error {
	...
	for _, unitName := range NormalizeSystemdUnitNames(units...) {
...
func (m *mockSystemdConnection) IsEnabled(ctx context.Context, unitName string) (bool, error) {
	...
	normalizedName := NormalizeSystemdUnitNames(unitName)[0]
	if unit, ok := m.units[normalizedName]; ok {
...
func (m *mockSystemdConnection) TryRestart(ctx context.Context, unitName string) error {
	...
	normalizedName := NormalizeSystemdUnitNames(unitName)[0]
	if unit, ok := m.units[normalizedName]; ok && unit.active {
...
func (m *mockSystemdConnection) Preset(ctx context.Context, unitName string) error {
	...
	normalizedName := NormalizeSystemdUnitNames(unitName)[0]
	if unit, ok := m.units[normalizedName]; ok {
		unit.enabled = m.presetPolicies[normalizedName]
	}
	return nil
}

func (m *mockSystemdConnection) SetPresetPolicy(unit string, enabled bool) {
	m.presetPolicies[NormalizeSystemdUnitNames(unit)[0]] = enabled
}
...
type mockSystemdManager struct {
	...
	// The callback can perform assertions, modify state, and return errors
	OnNewConnectionFunc func(context.Context) (SystemdConnection, error)
