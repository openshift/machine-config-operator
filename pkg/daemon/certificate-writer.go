...
func (dn *Daemon) syncControllerConfigHandler(key string) error {
	...
	if key != ctrlcommonconsts.ControllerConfigName {
	...
	controllerConfig, err := dn.ccLister.Get(ctrlcommonconsts.ControllerConfigName)
	if err != nil {
	...
	if currentNodeControllerConfigResource != controllerConfig.ObjectMeta.ResourceVersion || controllerConfig.Annotations[ctrlcommonconsts.ServiceCARotateAnnotation] == ctrlcommonconsts.ServiceCARotateTrue {
		...
		if controllerConfig.Annotations[ctrlcommonconsts.ServiceCARotateAnnotation] == ctrlcommonconsts.ServiceCARotateTrue && dn.node.Annotations[constants.ControllerConfigSyncServerCA] != controllerConfig.Annotations[ctrlcommonconsts.ServiceCARotateAnnotation] {
	...
	if dn.node.Annotations[constants.ControllerConfigSyncServerCA] != controllerConfig.Annotations[ctrlcommonconsts.ServiceCARotateAnnotation] {
		annos[constants.ControllerConfigSyncServerCA] = controllerConfig.Annotations[ctrlcommonconsts.ServiceCARotateAnnotation]
	...
	if controllerConfig.Annotations[ctrlcommonconsts.ServiceCARotateAnnotation] == ctrlcommonconsts.ServiceCARotateTrue && oldAnno != controllerConfig.Annotations[ctrlcommonconsts.ServiceCARotateAnnotation] && cmErr == nil && kubeConfigDiff && !allCertsThere && !dn.deferKubeletRestart {
...
func (dn *Daemon) syncOSImagePullSecrets(controllerConfig *mcfgv1.ControllerConfig) error {
	...
	if controllerConfig == nil {
		cfg, err := dn.ccLister.Get(ctrlcommonconsts.ControllerConfigName)
		if err != nil {
			return fmt.Errorf("could not get ControllerConfig: %v", err)
		}
