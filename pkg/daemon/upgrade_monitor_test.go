...
func (tc upgradeMonitorTestCase) run(t *testing.T) {
	...
	for _, n := range f.nodeLister {
		...
		err = upgrademonitor.GenerateAndApplyMachineConfigNodes(tc.parentCondition, tc.childCondition, tc.parentStatus, tc.childStatus, n, d.mcfgClient, d.featureGatesAccessor, mcpName, desiredConfigVersion)
		if err != nil {
