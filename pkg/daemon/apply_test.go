...
func TestStrategyLookup(t *testing.T) {
	...
	assert.Equal(t, err, fmt.Errorf("Default strategy for applying changes to \"/etc/containers/registries.conf\""))
...
func TestCalculateActions(t *testing.T) {
	tests := []struct {
		...
		expectedActions: []ConfigUpdateAction{
			RebootPostAction{Reason: "Default strategy for applying changes to \"/etc/containers/registries.conf\""},
		},
	}, {
		mConfig: "test-base",
