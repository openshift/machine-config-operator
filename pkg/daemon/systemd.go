...
type SystemdManager interface {
	...
	NewConnection(context.Context) (SystemdConnection, error)
...
func (s *SystemdManagerDefault) NewConnection(_ context.Context) (SystemdConnection, error) {
	return &systemdConnectionImpl{}, nil
}
...
func (s *systemdConnectionImpl) connect(ctx context.Context) (*systemddbus.Conn, error) {
	var err error
	s.connMutex.Lock()
	...
	s.conn, err = systemddbus.NewSystemdConnectionContext(ctx)
	if err != nil {
		err = fmt.Errorf("failed to connect to systemd: %w", err)
	}
...
func (s *systemdConnectionImpl) ReloadDaemon(ctx context.Context) error {
	return s.doWithConnection(ctx, func(conn *systemddbus.Conn) error {
...
func (s *systemdConnectionImpl) Enable(ctx context.Context, force bool, units ...string) error {
	return s.doWithConnection(ctx, func(conn *systemddbus.Conn) error {
		...
		if _, _, err := conn.EnableUnitFilesContext(ctx, normalizedUnits, false, force); err != nil {
...
func (s *systemdConnectionImpl) Disable(ctx context.Context, units ...string) error {
	return s.doWithConnection(ctx, func(conn *systemddbus.Conn) error {
		...
		if _, err := conn.DisableUnitFilesContext(ctx, normalizedUnits, false); err != nil {
...
func (s *systemdConnectionImpl) TryRestart(ctx context.Context, unit string) error {
	...
	if err := s.doWithConnection(ctx, doSync(ctx, func(c chan string, conn *systemddbus.Conn) error {
...
func (s *systemdConnectionImpl) IsEnabled(ctx context.Context, unit string) (bool, error) {
	var enabled bool
	err := s.doWithConnection(ctx, func(conn *systemddbus.Conn) error {
		normalizedName := NormalizeSystemdUnitNames(unit)[0]
		logSystem("Checking if systemd unit %q is enabled", normalizedName)
		enabledUnits, err := conn.ListUnitFilesByPatternsContext(
			ctx,
			[]string{"enabled", "enabled-runtime"},
			[]string{normalizedName},
		)
		if err != nil {
			return fmt.Errorf("checking if unit %q is enabled: %w", normalizedName, err)
		}
		enabled = len(enabledUnits) > 0
		logSystem("Checked systemd unit %q enabled=%v", normalizedName, enabled)
		return nil
	})
	return enabled, err
}
...
func (s *systemdConnectionImpl) ListUnits(ctx context.Context) (map[string]systemddbus.UnitStatus, error) {
	var result map[string]systemddbus.UnitStatus
	err := s.doWithConnection(ctx, func(conn *systemddbus.Conn) error {
		logSystem("Listing systemd units")
		units, err := conn.ListUnitsContext(ctx)
		if err != nil {
			return fmt.Errorf("getting units list: %w", err)
		}
...
func (s *systemdConnectionImpl) doWithConnection(ctx context.Context, fn func(*systemddbus.Conn) error) error {
...
// doSync wraps a D-Bus unit operation so it blocks until the resulting systemd
// job completes or the context is cancelled. The returned function is compatible
// with doWithConnection.
func doSync(ctx context.Context, fn func(chan string, *systemddbus.Conn) error) func(*systemddbus.Conn) error {
	return func(conn *systemddbus.Conn) error {
