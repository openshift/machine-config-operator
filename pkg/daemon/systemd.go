...
func (s *SystemdManagerDefault) NewConnection(ctx context.Context) (SystemdConnection, error) {
	return &systemdConnectionImpl{ctx: ctx}, nil
}
...
func (s *systemdConnectionImpl) connect() (*systemddbus.Conn, error) {
	s.connMutex.Lock()
	...
	var err error
	s.conn, err = systemddbus.NewSystemdConnectionContext(s.ctx)
	if err != nil {
...
func (s *systemdConnectionImpl) ReloadDaemon(ctx context.Context) error {
	return s.doWithConnection(func(conn *systemddbus.Conn) error {
...
func (s *systemdConnectionImpl) TryRestart(ctx context.Context, unit string) error {
	...
	if err := s.doWithConnection(doSync(ctx, func(c chan string, conn *systemddbus.Conn) error {
...
func (s *systemdConnectionImpl) IsEnabled(ctx context.Context, unit string) (bool, error) {
	var enabled bool
	err := s.doWithConnection(func(conn *systemddbus.Conn) error {
		normalizedName := NormalizeSystemdUnitNames(unit)[0]
...
func (s *systemdConnectionImpl) doWithConnection(fn func(*systemddbus.Conn) error) error {
	conn, err := s.connect()
	if err != nil {
		return err
	}
