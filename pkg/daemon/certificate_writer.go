...
func (dn *Daemon) syncControllerConfigHandler(key string) error {
	...
	if currentNodeControllerConfigResource != controllerConfig.ObjectMeta.ResourceVersion || controllerConfig.Annotations[ctrlcommon.ServiceCARotateAnnotation] == ctrlcommon.ServiceCARotateTrue {
		...
		if err := dn.syncClusterPullSecrets(controllerConfig); err != nil {
			return err
		}
...
func (dn *Daemon) syncClusterPullSecrets(controllerConfig *mcfgv1.ControllerConfig) error {
	dn.osImageMux.Lock()
	...
	if err := writeToDisk(map[string][]byte{imageRegistryAuthFile: controllerConfig.Spec.InternalRegistryPullSecret}); err != nil {
