...
type Daemon struct {
	...
	// BootcClient provides bootc-based OS updates when BootcNodeManagement feature gate is enabled
	BootcClient NodeUpdaterInterface
...
func New(
	...
	// Initialize bootc client for future use when BootcNodeManagement feature gate is enabled
	// We initialize it here but will only use it when the feature gate is checked
	if hostos.IsCoreOSVariant() {
		bootcClient = NewBootcClient()
		err := bootcClient.Initialize()
		if err != nil {
			klog.Warningf("Failed to initialize bootc client (this is expected on non-bootc systems): %v", err)
			// Don't fail daemon initialization if bootc is not available
			bootcClient = nil
		}
	}
