...
type Daemon struct {
	...
	// NodeOstreeClient wraps rpm-ostree and will eventually be removed with a direct rpmostreeclient value
	NodeOstreeClient *RpmOstreeClient

	// NodeBootcClient wraps bootc and will eventually be removed with a direct bootcclient value
	NodeBootcClient *BootcClient
...
func New(
	...
	var nodeOstreeClient *RpmOstreeClient

	// Only pull the osImageURL from OSTree when we are on RHCOS or FCOS
	if hostos.IsCoreOSVariant() {
		nodeOstreeClientVal := NewNodeOstreeClient()
		nodeOstreeClient = &nodeOstreeClientVal
		err := nodeOstreeClient.Initialize()
		if err != nil {
			return nil, fmt.Errorf("error initializing rpm-ostree: %w", err)
		}
		osImageURL, osVersion, osCommit, err = nodeOstreeClient.GetBootedOSImageURL()
		if err != nil {
	...
	return &Daemon{
		...
		rebootQueued:           false,
		os:                     hostos,
		NodeOstreeClient:       nodeOstreeClient,
...
func (dn *Daemon) getStateAndConfigs() (*stateAndConfigs, error) {
	...
	// fail.
	if dn.NodeOstreeClient != nil {
		finalizeError, err := dn.NodeOstreeClient.Peel().QueryPreviousDeploymentError()
		if err != nil {
...
func (dn *Daemon) LogSystemData() {
	...
	if dn.os.IsCoreOSVariant() {
		...
		// 	out, err := runGetOut("bootc", "status")
		// 	if err != nil {
		// 		klog.Fatalf("unable to get bootc status: %s", err)
		// 	}
		// 	klog.Infof("%s", out)
		// } else {
...
func (dn *Daemon) IsBootc() (bool, error) {
	fg, err := dn.featureGatesAccessor.CurrentFeatureGates()
	if err != nil {
		return false, err
	}
	return fg.Enabled(features.FeatureGateBootcNodeManagement), nil
}
