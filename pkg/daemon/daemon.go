...
func (dn *Daemon) generateBootstrappingMCMismatchError(currentConfigOnDisk *onDiskConfig, currentConfigName string) error {
	...
	dn.createBootstrapDiffFile(currentConfigOnDisk.currentConfig, inClusterGeneratedMC)
...
func (dn *Daemon) createBootstrapDiffFile(oldConfig, newConfig *mcfgv1.MachineConfig) {
	oldIgn, err := ctrlcommon.ParseAndConvertConfig(oldConfig.Spec.Config.Raw)
	if err != nil {
		klog.Infof("parsing old Ignition config failed with error: %v", err)
		return
	}
	newIgn, err := ctrlcommon.ParseAndConvertConfig(newConfig.Spec.Config.Raw)
	if err != nil {
		klog.Infof("parsing new Ignition config failed with error: %v", err)
		return
	}
	diffResult := cmp.Diff(oldIgn, newIgn)
	klog.Errorf("bootstrap generated MC %s and in-cluster generated MC %s diff:\n %s,", oldConfig.Name, newConfig.Name, diffResult)
	...
	if err := writeFileAtomicallyWithDefaults(bootstrapConfigDiffPath, []byte(diffResult)); err != nil {
		klog.Errorf("failed to write bootstrap diff config to %s: %v", bootstrapConfigDiffPath, err)
	} else {
		klog.Infof("bootstrap diff config written to %s", bootstrapConfigDiffPath)
	}
}
