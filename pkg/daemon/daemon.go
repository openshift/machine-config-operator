...
// stopKubeletCommand creates a new transient systemd unit to stop the kubelet.
// We explicitly try to stop kubelet.service first, before anything else; this
// way we ensure the rest of system stays running, because kubelet may need
// to do "graceful" shutdown by e.g. de-registering with a load balancer.
func stopKubeletCommand() *exec.Cmd {
	return exec.Command("systemd-run", "--wait", "--unit", "machine-config-daemon-stop-kubelet",
		"--description", "machine-config-daemon: stopping kubelet", "/bin/sh", "-c", "systemctl stop kubelet.service")
}
...
func rebootCommand(rationale string) *exec.Cmd {
	return exec.Command("systemd-run", "--unit", "machine-config-daemon-reboot",
		"--description", fmt.Sprintf("machine-config-daemon: %s", rationale), "/bin/sh", "-c", "reboot")
}
