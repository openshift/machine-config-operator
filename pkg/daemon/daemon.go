...
//
// As of recently, we opt-in to `--check-inhibitors=true` to allow system
// administrators to protect their systems from actually rebooting if that
// would be a problem.  We definitely want this to interoperate better with
...
func rebootCommand(rationale string) *exec.Cmd {
	return exec.Command("systemd-run", "--unit", "machine-config-daemon-reboot",
		"--description", fmt.Sprintf("machine-config-daemon: %s", rationale), "/bin/sh", "-c", "systemctl stop kubelet.service; systemctl reboot --check-inhibitors=true")
}
