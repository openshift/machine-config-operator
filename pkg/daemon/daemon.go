...
func (dn *Daemon) syncNode(key string) error {
	...
	if ufc != nil {
		...
		klog.Info("calling trigger update from syncnode")
...
func (dn *Daemon) runPreflightConfigDriftCheck() error {
	...
	klog.Infof("[runPreflightConfigDriftCheck] seeing if validate on disk state or image is correct")
