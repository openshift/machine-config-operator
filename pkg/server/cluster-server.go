...
func (cs *clusterServer) GetConfig(cr poolRequest) (*runtime.RawExtension, error) {
	...
	ignConf, err := ctrlcommonconfigs.ParseAndConvertConfig(mc.Spec.Config.Raw)
	if err != nil {
	...
	cc, err := cs.controllerConfigLister.Get(ctrlcommonconsts.ControllerConfigName)
	if err != nil {
	...
	if cs.configMapLister != nil {
		cm, err := cs.configMapLister.ConfigMaps(ctrlcommonconsts.MCONamespace).Get("kubeconfig-data")
		if err != nil {
