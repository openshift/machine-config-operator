...
type PinnedImageSetLister interface {
	...
	List(selector labels.Selector) (ret []*machineconfigurationv1.PinnedImageSet, err error)
	// Get retrieves the PinnedImageSet from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*machineconfigurationv1.PinnedImageSet, error)
...
type pinnedImageSetLister struct {
	listers.ResourceIndexer[*machineconfigurationv1.PinnedImageSet]
}
...
func NewPinnedImageSetLister(indexer cache.Indexer) PinnedImageSetLister {
	return &pinnedImageSetLister{listers.New[*machineconfigurationv1.PinnedImageSet](indexer, machineconfigurationv1.Resource("pinnedimageset"))}
}
