...
func getBuildController(ctx context.Context, cb *clients.Builder) ([]*build.Controller, error) {
	...
	for _, mosc := range machineOSConfigs.Items {
		if mosc.Spec.BuildInputs.ImageBuilderType == mcfgv1alpha1.DefaultBuilder || mosc.Spec.BuildInputs.ImageBuilderType == mcfgv1alpha1.OCPBuilder {
			controllersToStart = append(controllersToStart, build.NewWithImageBuilder(cfg, buildClients))
		} else if mosc.Spec.BuildInputs.ImageBuilderType == mcfgv1alpha1.PodBuilder {
