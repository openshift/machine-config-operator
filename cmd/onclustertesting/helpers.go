...
func teardownPool(cs *framework.ClientSet, mcp *mcfgv1.MachineConfigPool) error {
	if err := cs.MachineConfigPools().Delete(context.TODO(), mcp.Name, metav1.DeleteOptions{}); err != nil {
		return err
	}
...
func deleteAllMachineConfigsForPool(cs *framework.ClientSet, targetPool string) error {
	...
	for _, mc := range machineConfigs.Items {
		if _, ok := mc.Annotations[helpers.MCPNameToRole(targetPool)]; ok && !strings.HasPrefix(mc.Name, "rendered-") {
...
func deleteBuildObjects(cs *framework.ClientSet, mcp *mcfgv1.MachineConfigPool) error {
	return deleteBuildObjectsForRenderedMC(cs, mcp.Spec.Configuration.Name)
}

func deleteBuildObjectsForRenderedMC(cs *framework.ClientSet, renderedMC string) error {
	listOpts := metav1.ListOptions{
		LabelSelector: fmt.Sprintf("machineconfiguration.openshift.io/desiredConfig=%s", renderedMC),
	}
...
func common(opts interface{}) {
	...
	klog.V(2).Infof("Options parsed: %+v", opts)
