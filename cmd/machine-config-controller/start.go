...
func startControllers(ctx *common.ControllerContext) error {
	...
	// Add all "sub-renderers here".  If any of them are *required*, then
	// you should add a readyFlag for them and notify when their fragment is ready.
	// By default there's no `kubeletconfig`, so we don't need to block on this today.
	...
	// Async wait for the MC fragments before starting the renderer
	go func() {
		readyFlag.Wait()
